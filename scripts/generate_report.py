...
corp_counts = Counter(s[1] for s in unmatched)

report = f"""# MEXT School Code Matching Report
...
- Total schools in our list: **{results['total']}**
...
|----------|---------|------|
| A. Exact name match | {results['exact_count']} | {results['exact_rate']:.1f}% |
| B. Fuzzy match (NFKC normalization) | +{results['fuzzy_count']} | +{results['fuzzy_rate']:.1f}% |
| C. Prefecture + partial name | +{results['partial_count']} | +{results['partial_rate']:.1f}% |
| **Cumulative** | **{results['exact_count'] + results['fuzzy_count'] + results['partial_count']}** | **{results['cumulative_rate']:.1f}%** |
| Unmatched | {results['unmatched_count']} | {results['unmatched_rate']:.1f}% |
...
### Unmatched Schools Analysis ({results['unmatched_count']} schools)

Of the {results['unmatched_count']} unmatched schools:
...
for corp, count in corp_counts.most_common(10):
    report += f"| {corp} | {count} |\n"
