...
def test_parse_pdf_uses_table_grid_metrics_for_gakusei_labels(monkeypatch, tmp_path: Path) -> None:
    table = [
    ...
    annotation = parse_pdf(tmp_path / "ohara.pdf")

    assert len(annotation.departments) == 1
    dept = annotation.departments[0]
    assert dept.name == "ビジネスキャリア2年制"
    assert dept.course_name == "専門課程"
    assert dept.duration_years == 2
    assert dept.day_or_evening == "昼"
    assert dept.capacity == 140
    assert dept.enrollment == 113
    assert dept.intl_students == 0
