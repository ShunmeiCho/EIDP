...
def test_verify_core_zip_requires_publication_lag_release_exception_contract(tmp_path: Path) -> None:
    ...
    entries["scripts/verify_stage6_return.py"] = (
        ...
        .replace("release exception record Approval date is required", "approval date optional")
        .replace("Approval date must be on or after last_run finished_at date", "approval may predate last_run")
        ...
        .replace("example owner", "demo owner")
    )
    ...
    assert any(
        ...
        "mature-year proof case fiscal_year must be an integer" in error
        for error in check.errors
    )
    assert any(
        "scripts/verify_stage6_return.py missing required token: "
    ...
    assert any(
        "scripts/verify_stage6_return.py missing required token: KPI actual must match" in error
        for error in check.errors
    )
    assert any(
    ...
    assert any(
        "scripts/verify_stage6_return.py missing required token: "
        "mature-year proof years must match passing proof JSON years" in error
        for error in check.errors
    )
    assert any(
    ...
    assert any(
        "scripts/verify_stage6_return.py missing required token: RELEASE_CONCLUSIONS" in error
        for error in check.errors
    )
    assert any(
        "scripts/verify_stage6_return.py missing required token: RC_ONLY" in error
        for error in check.errors
    )
    assert any(
    ...
    assert any(
        "scripts/verify_stage6_return.py missing required token: release exception record Approval date is required"
        in error
        for error in check.errors
    )
    assert any(
        "scripts/verify_stage6_return.py missing required token: "
        "Approval date must be on or after last_run finished_at date" in error
        for error in check.errors
    )
    assert any(
    ...
    assert any(
        "scripts/verify_stage6_return.py missing required token: example owner" in error
        for error in check.errors
    )
    assert any(
        "scripts/verify_stage6_return.py missing required token: "
        "Approval date must be on or after mature-year proof finished_at date" in error
        for error in check.errors
    )
    assert any(
