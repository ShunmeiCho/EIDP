...
def test_summarize_discovery_gold_entries_tracks_release_relevant_buckets() -> None:
    ...
    assert summary.outcome_counts == {
        "accepted_target_pdf": 8,
        "needs_operator_review": 16,
        "no_target_candidate_found": 1,
        "publication_lag_latest_public": 11,
        "site_fetch_error": 1,
    }
    assert summary.strict_target_year_successes == 8
    assert summary.operator_review_entries == 16
    ...
    assert "commented_old_pdf_with_visible_target_form_without_year_evidence" in summary.site_families
...
def test_render_discovery_gold_summary_outputs_json_safe_payload() -> None:
    ...
    assert decoded["outcome_counts"]["needs_operator_review"] == 16
    ...
    assert decoded["strict_target_year_successes"] == 8
