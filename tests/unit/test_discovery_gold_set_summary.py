...
def test_summarize_discovery_gold_entries_tracks_release_relevant_buckets() -> None:
    ...
    assert summary.total_entries == 25
    assert summary.target_fiscal_year_counts == {2025: 2, 2026: 23}
    assert summary.outcome_counts == {
        "accepted_target_pdf": 4,
        "needs_operator_review": 10,
        "no_target_candidate_found": 1,
        "publication_lag_latest_public": 9,
        "site_fetch_error": 1,
    }
    assert summary.strict_target_year_successes == 4
    assert summary.operator_review_entries == 10
    assert summary.publication_lag_entries == 9
...
def test_render_discovery_gold_summary_outputs_json_safe_payload() -> None:
    ...
    assert decoded["total_entries"] == 25
    assert decoded["outcome_counts"]["needs_operator_review"] == 10
    assert decoded["outcome_counts"]["no_target_candidate_found"] == 1
    assert decoded["outcome_counts"]["publication_lag_latest_public"] == 9
