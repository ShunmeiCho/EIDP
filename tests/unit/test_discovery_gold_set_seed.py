...
def test_seed_discovery_gold_sites_cli_applies_when_requested(monkeypatch) -> None:
    ...
    assert payload["schools_to_create"] == 35
    assert payload["sites_to_add"] == 35
    assert payload["invalid_site_urls"] == 0
    with Session(engine) as session:
        assert session.query(SchoolSite).filter(SchoolSite.discovery_method == "discovery_gold_set").count() == 35
