...
def test_run_pdf_discovery_prioritizes_trusted_disclosure_page_over_homepage(
    monkeypatch, tmp_path: Path
) -> None:
    session = _session()
    crawled_urls: list[str] = []
...
def test_run_pdf_discovery_skips_corporation_pattern_when_school_override_exists(
    monkeypatch, tmp_path: Path
) -> None:
    session = _session()
    crawled_urls: list[str] = []
    ...
    try:
        session.add_all(
            [
                ...
                SchoolSite(
                    school_id=1,
                    url="https://www.neec.ac.jp/portal/public/mext-scholarship/",
                    url_type="school",
                    discovery_method="school_domain_override",
                    http_status=200,
                    confidence=0.98,
                ...
                SchoolSite(
                    school_id=2,
                    url="https://www.other-group.example/",
                    url_type="corporation",
                    discovery_method="corporation_pattern",
                    http_status=200,
                    confidence=0.5,
...
def test_run_pdf_discovery_keeps_slug_disclosure_probe_for_shared_origin(
    monkeypatch,
    tmp_path: Path,
) -> None:
    """Shared-origin throttling must keep per-school /disclosure/{slug} probes."""
...
def test_download_pdf_accepts_image_only_specific_form_with_target_year_hint(
    monkeypatch, tmp_path: Path
) -> None:
    url = "https://example.ac.jp/school/info-2025/koutou-kyuhu.pdf"
    client = _AttemptPdfClient(
        {
            url: _AttemptPdfResponse(url, status_code=200, content=b"%PDF-" + (b"x" * 2000)),
        }
...
def test_download_pdf_prefers_target_url_hint_over_operational_history_year(
    monkeypatch, tmp_path: Path
) -> None:
    content = _make_pdf_bytes(
        "様式第2号 高等教育の修学支援新制度 確認申請書 機関要件 学科名 生徒総定員\n"
    ...
    candidate = PdfCandidate(
        pdf_url="https://www.bio.ac.jp/school/kakuninshinseisyo/youshiki_2025.pdf",
        page_url="https://www.bio.ac.jp/school/",
        anchor_text="令和七年度申請書（様式第２号）",
    )
...
def test_download_pdf_rejects_stale_body_document_year_despite_target_url_hint(
    monkeypatch, tmp_path: Path
) -> None:
    content = _make_pdf_bytes("2024年度 高等教育の修学支援新制度 確認申請書 機関要件 学科名 生徒総定員")
    candidate = PdfCandidate(
...
def test_download_pdf_accepts_form_adjacent_bare_year_when_body_is_target_form(
    monkeypatch, tmp_path: Path
) -> None:
    content = _make_pdf_bytes("高等教育の修学支援新制度 確認申請書 機関要件 学科名 生徒総定員")
    candidate = PdfCandidate(
        pdf_url="https://example.ac.jp/disclosure/yoshiki2025.pdf",
        page_url="https://example.ac.jp/disclosure/",
        anchor_text="申請様式2025",
    )
...
def test_download_pdf_accepts_school_domain_override_embedded_studyspt_target_form(
    ...
    content = _make_pdf_bytes("高等教育の修学支援新制度 様式第2号 機関要件 学科名 生徒総定員")
    candidate = PdfCandidate(
        pdf_url="https://www.nkz.ac.jp/clginfo/tm/pdf/tmZ-studyspt_13.pdf",
        page_url="https://www.nkz.ac.jp/clginfo/tm/tmZ-studyspt_13.html",
        anchor_text="",
        pattern_type="embed",
    )
...
def test_download_pdf_accepts_trusted_prefecture_specific_yearless_target_form(
    monkeypatch, tmp_path: Path
) -> None:
    content = _make_pdf_bytes("高等教育の修学支援新制度 様式第2号 機関要件 学科名 生徒総定員")
    candidate = PdfCandidate(
...
def test_download_pdf_accepts_trusted_prefecture_confirmation_application_url(
    monkeypatch, tmp_path: Path
) -> None:
    content = _make_pdf_bytes("高等教育の修学支援新制度 様式第2号 機関要件 学科名 生徒総定員")
    candidate = PdfCandidate(
        pdf_url="https://www.tca.ac.jp/school/public_info/data/2025/11_confirmation_application.pdf?20250903",
        page_url="https://www.tca.ac.jp/school/public_info/",
        anchor_text="大学等における修学の支援・高等教育修学支援制度",
    )
    candidate.trusted_year_evidence = "prefecture_index_current_year"
...
def test_download_pdf_accepts_trusted_prefecture_embedded_studyspt_target_form(
    monkeypatch, tmp_path: Path
) -> None:
    content = _make_pdf_bytes("高等教育の修学支援新制度 様式第2号 機関要件 学科名 生徒総定員")
    candidate = PdfCandidate(
        pdf_url="https://www.nkz.ac.jp/clginfo/nh/pdf/nhZ-studyspt_13.pdf",
        page_url="https://www.nkz.ac.jp/clginfo/nh/nhZ-studyspt_13.html",
        anchor_text="",
        pattern_type="embed",
    )
    candidate.trusted_year_evidence = "prefecture_index_current_year"
