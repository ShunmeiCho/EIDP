...
def _fixture_layout(tmp_path: Path) -> tuple[Path, Path]:
    ...
    (tessdata / "configs" / "tsv").write_text("tessedit_create_tsv 1\n", encoding="utf-8")
    (tessdata / "configs" / "pdf").write_text("ignored", encoding="utf-8")
...
def test_collect_ocr_addon_members_matches_runtime_layout(tmp_path: Path) -> None:
    ...
    assert "ocr-addon/tessdata/configs/tsv" in arcs
    assert "ocr-addon/tessdata/configs/pdf" not in arcs
...
def test_build_ocr_addon_zip_writes_manifest_and_expected_files(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(out_zip) as zf:
        ...
        assert "ocr-addon/tessdata/configs/tsv" in names
    ...
    assert manifest["required"]["tsv_config"] == "ocr-addon/tessdata/configs/tsv"
    ...
    assert "ocr-addon/tessdata/configs/tsv" in manifest_paths
...
def test_ocr_addon_requires_tsv_config(tmp_path: Path) -> None:
    tess, tessdata = _fixture_layout(tmp_path)
    (tessdata / "configs" / "tsv").unlink()

    with pytest.raises(OcrAddonError, match="configs.*tsv"):
        collect_ocr_addon_members(tesseract_dir=tess, tessdata_dir=tessdata)
