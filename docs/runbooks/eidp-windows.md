...
`学校キュー` の見出し直下に、次のような実行中パッケージ情報が表示されます。
...
`学校キュー` ページの `初回URL/PDF取得を開始` ボタンを押してください。
...
に保存します。
未対応の都道府県や未掲載校だけ、`学校キュー` の `URL追加`
から公式の情報公開ページを補足してください。
学校数が多いため、完了まで数十分かかることがあります。

業務でよく使うページ:

- `学校キュー` — 学校ごとの対象年度PDFの進捗と次に行う作業の確認
- `申請書PDF確認` — 画像 PDF、低信頼度、解析失敗、旧年度fallback の確認と手入力
- `対象年度確認` — 年度判定が誤っている PDF の年度修正
- `Excel出力` — 出力前確認と Excel ダウンロード
...
Excel は、業務員が画面で確認した後に `Excel出力` ページから作成します。
...
通常は先に `学校キュー` ページを開きます。
学校ごとに `次の作業` を確認し、`PDF確認`、`OCR/手入力`、`手入力` になっている学校だけ
`申請書PDF確認` ページで作業します。

`申請書PDF確認` ページでは、対象年度PDFと旧年度fallbackを区別して表示します。
...
1. `学校キュー` で対象の学校と `次の作業` を確認します。
2. `申請書PDF確認` で同じ学校の PDF を選びます。
...
`Excel出力` ページで Excel の内容を確認してから出力します。
...
- 手動で PDF を入手できる場合は `申請書PDF確認` で対応します。
...
| 学内 proxy / SSL inspection | PDF discovery の outbound HTTP(S) が失敗する | ブラウザで学校サイト PDF を手動確認し、proxy / allowlist が必要なら管理者に依頼する。HTTPX は標準の `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY` を既定で読む。自動取得できないものは `申請書PDF確認` に回す |
...
SSRF 防御により安全側で拒否され、`申請書PDF確認` に回ることがある。これは公開校 PDF の
...
| Mac から `ssh -N -L 127.0.0.1:18501:127.0.0.1:8501 win` を起動しても、Mac 側で `127.0.0.1:18501` が LISTEN しない | local `Host win` の SSH config で `ClearAllForwardings yes` が有効だと、コマンドラインの `-L` が消される | `EIDP-start.bat` / `scripts\launch.bat` の既定ポートは `8501`。tunnel 検証時は明示的に上書きする:<br><br>`ssh -N -o ClearAllForwardings=no -o ExitOnForwardFailure=yes -L 127.0.0.1:18501:127.0.0.1:8501 win`<br><br>起動後、Mac 側で `curl http://127.0.0.1:18501/_stcore/health` が `ok` を返すことを確認してから Playwright を開く。手動で Streamlit を `--server.port 8502` 起動した検証だけ、ローカル `18502` から Windows `8502` へ転送する:<br><br>`ssh -N -o ClearAllForwardings=no -o ExitOnForwardFailure=yes -L 127.0.0.1:18502:127.0.0.1:8502 win`<br><br>その場合は `curl http://127.0.0.1:18502/_stcore/health` で確認する。 |
