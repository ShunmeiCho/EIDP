...
- 管理者が明示的に固定したい場合だけ `.env` の `EIDP_TARGET_FISCAL_YEAR` を使います。
...
この操作で対応済みの都道府県公開データから学校URLを登録し、対象年度PDFの探索を開始します。
現在の初回取得は専門学校中心です。大学は未取得が残る前提で、`学校別タスク` の `URL追加`
...
- 登録済みの都道府県由来URLと、業務員が追加した公式URLから新しい対象年度 PDF を探します。
