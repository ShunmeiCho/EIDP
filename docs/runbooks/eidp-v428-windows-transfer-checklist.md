# EIDP v428 Windows Transfer Checklist
...
is intentionally scoped to v428 and should not be used as evidence by itself;
...
| Field | Value |
| --- | --- |
| ZIP | `dist/eidp-windows-v428.zip` |
...
| Optional local manifest | `dist/eidp-windows-v428-transfer-manifest.json` |
| Suggested staging path | `C:\EIDP-staging\` |
| Suggested extract path | `C:\Users\cyo20\EIDP-v428-e15c9e12` |

## Companion Docs

The v428 ZIP contains the packaged Windows app and the reusable operator E2E
...
- `docs/reports/eidp-v428-stage6-evidence-draft.md`

Use the ZIP-embedded `docs/runbooks/eidp-operator-e2e-template.md` as the blank
recording form. Copy v428 package, SHA256, gate, and Windows execution values
...
```bash
shasum -a 256 -c dist/eidp-windows-v428.zip.sha256
if [ -f dist/eidp-windows-v428-transfer-manifest.json ]; then
  python -m json.tool dist/eidp-windows-v428-transfer-manifest.json >/dev/null
fi
uv run python scripts/run_non_windows_release_gates.py \
  dist/eidp-windows-v428.zip \
  --skip-full-unit \
  --allow-docs-only-stale-package \
  --json \
  --output logs/release-gate-v428-docs-only-stale-before-windows-transfer.json
```

Expected:

- `shasum` reports `dist/eidp-windows-v428.zip: OK`.
...
scp dist/eidp-windows-v428.zip dist/eidp-windows-v428.zip.sha256 win:C:/EIDP-staging/
```
...
- `dist/eidp-windows-v428.zip`
- `dist/eidp-windows-v428.zip.sha256`
...
Copy-Item -LiteralPath (Join-Path $Source "eidp-windows-v428.zip") -Destination "C:\EIDP-staging\eidp-windows-v428.zip"
Copy-Item -LiteralPath (Join-Path $Source "eidp-windows-v428.zip.sha256") -Destination "C:\EIDP-staging\eidp-windows-v428.zip.sha256"
Get-ChildItem C:\EIDP-staging\eidp-windows-v428.zip*
```
...
```powershell
$Expected = "fdca644833fdfabd728d4cf774f43ebf48521b3655c62ae9ea52b33778c1951e"
$Zip = "C:\EIDP-staging\eidp-windows-v428.zip"
...
```powershell
certutil -hashfile C:\EIDP-staging\eidp-windows-v428.zip SHA256
```
...
```powershell
$Zip = "C:\EIDP-staging\eidp-windows-v428.zip"
$Dest = "C:\Users\cyo20\EIDP-v428-e15c9e12"
if (Test-Path $Dest) {
...
```powershell
Set-Location "C:\Users\cyo20\EIDP-v428-e15c9e12"
.\EIDP-setup.bat
```
...
- scheduled task points to this v428 extraction unless intentionally skipped
...
```bash
scp win:C:/Users/cyo20/EIDP-v428-e15c9e12/logs/stage6-evidence-*.zip logs/
latest_bundle="$(ls -t logs/stage6-evidence-*.zip | head -n 1)"
test -n "$latest_bundle" || { echo "No logs/stage6-evidence-*.zip found"; exit 1; }
verify_json="logs/stage6-evidence-verify-mac-v428-$(date +%Y%m%d-%H%M%S).json"
...
- `C:\Users\cyo20\EIDP-v428-e15c9e12\logs\stage6-evidence-*.zip`
- `C:\Users\cyo20\EIDP-v428-e15c9e12\logs\stage6-evidence-verify-*.json`
- `C:\Users\cyo20\EIDP-v428-e15c9e12\logs\run-*.log`
- `C:\Users\cyo20\EIDP-v428-e15c9e12\data\output\last_run.json`
