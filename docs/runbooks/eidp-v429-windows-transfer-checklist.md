# EIDP v429 Windows Transfer Checklist
...
is intentionally scoped to v429 and should not be used as evidence by itself;
...
| Field | Value |
| --- | --- |
| ZIP | `dist/eidp-windows-v429.zip` |
...
| Optional local manifest | `dist/eidp-windows-v429-transfer-manifest.json` |
| Suggested staging path | `C:\EIDP-staging\` |
| Suggested extract path | `C:\Users\cyo20\EIDP-v429-bdf7f421` |

## Companion Docs

The v429 ZIP contains the packaged Windows app and the reusable operator E2E
...
- `docs/reports/eidp-v429-stage6-evidence-draft.md`

Use the ZIP-embedded `docs/runbooks/eidp-operator-e2e-template.md` as the blank
recording form. Copy v429 package, SHA256, gate, and Windows execution values
...
```bash
shasum -a 256 -c dist/eidp-windows-v429.zip.sha256
if [ -f dist/eidp-windows-v429-transfer-manifest.json ]; then
  python -m json.tool dist/eidp-windows-v429-transfer-manifest.json >/dev/null
fi
uv run python scripts/run_non_windows_release_gates.py \
  dist/eidp-windows-v429.zip \
  --skip-full-unit \
  --allow-docs-only-stale-package \
  --json \
  --output logs/release-gate-v429-docs-only-stale-before-windows-transfer.json
```

Expected:

- `shasum` reports `dist/eidp-windows-v429.zip: OK`.
...
scp dist/eidp-windows-v429.zip dist/eidp-windows-v429.zip.sha256 win:C:/EIDP-staging/
```
...
- `dist/eidp-windows-v429.zip`
- `dist/eidp-windows-v429.zip.sha256`
...
Copy-Item -LiteralPath (Join-Path $Source "eidp-windows-v429.zip") -Destination "C:\EIDP-staging\eidp-windows-v429.zip"
Copy-Item -LiteralPath (Join-Path $Source "eidp-windows-v429.zip.sha256") -Destination "C:\EIDP-staging\eidp-windows-v429.zip.sha256"
Get-ChildItem C:\EIDP-staging\eidp-windows-v429.zip*
```
...
```powershell
$Expected = "b3acf94ec6dc3f7cd69496972a75e5eec4915e6adc8fdda7d49fb2d1e2c2de45"
$Zip = "C:\EIDP-staging\eidp-windows-v429.zip"
...
```powershell
certutil -hashfile C:\EIDP-staging\eidp-windows-v429.zip SHA256
```
...
```powershell
$Zip = "C:\EIDP-staging\eidp-windows-v429.zip"
$Dest = "C:\Users\cyo20\EIDP-v429-bdf7f421"
if (Test-Path $Dest) {
...
```powershell
Set-Location "C:\Users\cyo20\EIDP-v429-bdf7f421"
.\EIDP-setup.bat
```
...
- scheduled task points to this v429 extraction unless intentionally skipped
...
```bash
scp win:C:/Users/cyo20/EIDP-v429-bdf7f421/logs/stage6-evidence-*.zip logs/
latest_bundle="$(ls -t logs/stage6-evidence-*.zip | head -n 1)"
test -n "$latest_bundle" || { echo "No logs/stage6-evidence-*.zip found"; exit 1; }
verify_json="logs/stage6-evidence-verify-mac-v429-$(date +%Y%m%d-%H%M%S).json"
...
- `C:\Users\cyo20\EIDP-v429-bdf7f421\logs\stage6-evidence-*.zip`
- `C:\Users\cyo20\EIDP-v429-bdf7f421\logs\stage6-evidence-verify-*.json`
- `C:\Users\cyo20\EIDP-v429-bdf7f421\logs\run-*.log`
- `C:\Users\cyo20\EIDP-v429-bdf7f421\data\output\last_run.json`
