# EIDP v542 Owner Return Fill Sheet

Date: 2026-06-21
Scope: v542 owner/operator return evidence only
...
3. the operator runs one real Stage 6 cycle on the v542 side-by-side lane; and
...
| E2E field | Value |
| --- | --- |
| EIDP package commit | `d98ecd7196631a00c27aff1c240ebc7969579ce7` |
| core ZIP ファイル名 | `eidp-windows-v542.zip` |
| core ZIP sha256 | `89ace547fcabf43f80b697024f5c13d1398244ad4d4b165160a489c8386f9ecc` |
| transferred ZIP | `C:\EIDP-staging\eidp-windows-v542.zip` |
| Windows extract path | `%USERPROFILE%\EIDP-v542-d98ecd7-env0` |
| current active root to preserve | `%USERPROFILE%\EIDP-v527-69fe81f-env0` |
| non-Windows release gate | `logs/win-v542-false-reject-verifier-release-gates-20260621.json` -> `ok=true` |
| GitHub main CI | packaged source commit d98ecd7: CI `27880148454` -> success; v542 evidence-docs commit ba161f4: CI `27881613723` -> success; re-check current `main` before release approval |
| Windows validation evidence | `docs/reports/2026-06-21-v542-false-reject-verifier-windows-canary.md` |
| OCR runtime evidence | v542 OCR runtime validation is not complete; required if OCR remains in v1.0 scope |
| setup validation evidence | `docs/reports/2026-06-21-v542-false-reject-verifier-windows-canary.md` records after-setup validator `ok=true` |
| after-weekly validation evidence | `docs/reports/2026-06-21-v542-false-reject-verifier-windows-canary.md` records after-weekly validator `ok=true` |
| bounded canary evidence | strict/Excel-ready `12/50 (24.0%)`, operator-reviewable `47/50 (94.0%)`, `ship_gate_status=below_gate` |
| Stage 6 evidence verify | `logs/win-v542-d98ecd7-canary/stage6-evidence-verify-20260621-040959.json` -> `ok=true` |
...
The v542 bounded canary is side-by-side smoke evidence only. It is not owner
real-cycle sign-off evidence.

## Short Owner Decision Form

Use `docs/runbooks/eidp-v542-release-summary.md` as the owner-facing one-page
summary and `docs/runbooks/eidp-v542-owner-signoff.md` as the short owner
decision form. Do not ask the owner to manually fill or reproduce the
engineering checklist.

For v542, current evidence supports `NOT_READY` only. `publication_lag`
...
- v542 setup validation evidence
- v542 active-task recovery JSON
...
Developer validation is run from current `main`, or from a v542+ package
carrying the same verifier integration, after the returned CSV is copied back
to the repo. The v542 package proves the verifier is available; it does not
...
uv run python scripts/verify_stage6_return.py \
  ...
  --owner-signoff docs/runbooks/eidp-v542-owner-signoff.md \
  --expected-package-sha256 89ace547fcabf43f80b697024f5c13d1398244ad4d4b165160a489c8386f9ecc \
  --expected-source-commit d98ecd7196631a00c27aff1c240ebc7969579ce7 \
  --false-reject-evidence-zip logs/win-v542-d98ecd7-canary/stage6-evidence-20260620-190958.zip \
  --false-reject-review-csv docs/reports/2026-06-21-v542-false-reject-review-sheet.csv \
...
uv run python scripts/build_false_reject_audit.py \
  ...
  --sample-size 12 \
  --validate-review-csv docs/reports/2026-06-21-v542-false-reject-review-sheet.csv \
  --require-decisions
```
