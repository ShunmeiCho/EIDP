# EIDP v547 Owner Return Fill Sheet

Date: 2026-06-21
Scope: v547 owner/operator return evidence only
...
3. the operator runs one real Stage 6 cycle on the v547 side-by-side lane; and
...
| E2E field | Value |
| --- | --- |
| EIDP package commit | `86c848f68e1dbde85c9b6422cfc827149940e02a` |
| core ZIP ファイル名 | `eidp-windows-v547.zip` |
| core ZIP sha256 | `f167e17b89f0ff96a45c817abcfd0403a2d487eddf3fb3a85a73d866b351de4b` |
| transferred ZIP | `C:\EIDP-staging\eidp-windows-v547.zip` |
| Windows extract path | `%USERPROFILE%\EIDP-v547-86c848f-env0` |
| current active root to preserve | `%USERPROFILE%\EIDP-v527-69fe81f-env0` |
| non-Windows release gate | `logs/eidp-windows-v547-release-gates-20260621.json` -> `ok=true` |
| GitHub main CI | packaged source commit 86c848f: CI `27894031180` -> success; docs-only handoff commits do not change packaged runtime; re-check current main CI before release approval |
| Windows validation evidence | `docs/reports/2026-06-21-v547-windows-canary.md` |
| OCR runtime evidence | v547 OCR runtime validation is not complete; required if OCR remains in v1.0 scope |
| setup validation evidence | `docs/reports/2026-06-21-v547-windows-canary.md` records after-setup validator `ok=true` |
| after-weekly validation evidence | `docs/reports/2026-06-21-v547-windows-canary.md` records after-weekly validator `ok=true` |
| bounded canary evidence | strict/Excel-ready `12/50 (24.0%)`, operator-reviewable `47/50 (94.0%)`, `ship_gate_status=below_gate` |
| Stage 6 evidence verify | `logs/win-v547-86c848f-canary/stage6-evidence-verify-20260621-144556.json` -> `ok=true` |
...
The v547 bounded canary is side-by-side smoke evidence only. It is not owner
real-cycle sign-off evidence.

## Short Owner Decision Form

Use `docs/runbooks/eidp-v547-release-summary.md` as the owner-facing one-page
summary and `docs/runbooks/eidp-v547-owner-signoff.md` as the short owner
decision form. Do not ask the owner to manually fill or reproduce the
engineering checklist.

For v547, current evidence supports `NOT_READY` only. `publication_lag`
...
- v547 setup validation evidence
- v547 active-task recovery JSON
...
uv run python scripts/verify_stage6_return.py \
  ...
  --owner-signoff docs/runbooks/eidp-v547-owner-signoff.md \
  --expected-package-sha256 f167e17b89f0ff96a45c817abcfd0403a2d487eddf3fb3a85a73d866b351de4b \
