# EIDP v422 Windows Transfer Checklist
...
is intentionally scoped to v422 and should not be used as evidence by itself;
...
| Field | Value |
| --- | --- |
| ZIP | `dist/eidp-windows-v422.zip` |
...
| Optional local manifest | `dist/eidp-windows-v422-transfer-manifest.json` |
| Suggested staging path | `C:\EIDP-staging\` |
| Suggested extract path | `C:\Users\cyo20\EIDP-v422-fb5d4485` |

## Companion Docs

The v422 ZIP contains the packaged Windows app and the reusable operator E2E
...
- `docs/reports/eidp-v422-stage6-evidence-draft.md`

Use the ZIP-embedded `docs/runbooks/eidp-operator-e2e-template.md` as the blank
recording form. Copy v422 package, SHA256, gate, and Windows execution values
...
```bash
shasum -a 256 -c dist/eidp-windows-v422.zip.sha256
if [ -f dist/eidp-windows-v422-transfer-manifest.json ]; then
  python -m json.tool dist/eidp-windows-v422-transfer-manifest.json >/dev/null
fi
uv run python scripts/run_non_windows_release_gates.py \
  dist/eidp-windows-v422.zip \
  --skip-full-unit \
  --allow-docs-only-stale-package \
  --json \
  --output logs/release-gate-v422-docs-only-stale-before-windows-transfer.json
```

Expected:

- `shasum` reports `dist/eidp-windows-v422.zip: OK`.
...
scp dist/eidp-windows-v422.zip dist/eidp-windows-v422.zip.sha256 win:C:/EIDP-staging/
```
...
- `dist/eidp-windows-v422.zip`
- `dist/eidp-windows-v422.zip.sha256`
...
Copy-Item -LiteralPath (Join-Path $Source "eidp-windows-v422.zip") -Destination "C:\EIDP-staging\eidp-windows-v422.zip"
Copy-Item -LiteralPath (Join-Path $Source "eidp-windows-v422.zip.sha256") -Destination "C:\EIDP-staging\eidp-windows-v422.zip.sha256"
Get-ChildItem C:\EIDP-staging\eidp-windows-v422.zip*
```
...
```powershell
$Expected = "772591f5909ff6e77ca514f67d0a3cabb0b1e51b16db228562d8219b375e204b"
$Zip = "C:\EIDP-staging\eidp-windows-v422.zip"
...
```powershell
certutil -hashfile C:\EIDP-staging\eidp-windows-v422.zip SHA256
```
...
```powershell
$Zip = "C:\EIDP-staging\eidp-windows-v422.zip"
$Dest = "C:\Users\cyo20\EIDP-v422-fb5d4485"
if (Test-Path $Dest) {
...
```powershell
Set-Location "C:\Users\cyo20\EIDP-v422-fb5d4485"
.\EIDP-setup.bat
```
...
- scheduled task points to this v422 extraction unless intentionally skipped
...
```bash
scp win:C:/Users/cyo20/EIDP-v422-fb5d4485/logs/stage6-evidence-*.zip logs/
latest_bundle="$(ls -t logs/stage6-evidence-*.zip | head -n 1)"
test -n "$latest_bundle" || { echo "No logs/stage6-evidence-*.zip found"; exit 1; }
verify_json="logs/stage6-evidence-verify-mac-v422-$(date +%Y%m%d-%H%M%S).json"
...
- `C:\Users\cyo20\EIDP-v422-fb5d4485\logs\stage6-evidence-*.zip`
- `C:\Users\cyo20\EIDP-v422-fb5d4485\logs\stage6-evidence-verify-*.json`
- `C:\Users\cyo20\EIDP-v422-fb5d4485\logs\run-*.log`
- `C:\Users\cyo20\EIDP-v422-fb5d4485\data\output\last_run.json`
