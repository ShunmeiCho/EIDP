# EIDP v436 Windows Transfer Checklist
...
is intentionally scoped to v436 and should not be used as evidence by itself;
...
| Field | Value |
| --- | --- |
| ZIP | `dist/eidp-windows-v436.zip` |
...
| Optional local manifest | `dist/eidp-windows-v436-transfer-manifest.json` |
| Suggested staging path | `C:\EIDP-staging\` |
| Suggested extract path | `C:\Users\cyo20\EIDP-v436-f56b2e4b` |

## Companion Docs

The v436 ZIP contains the packaged Windows app and the reusable operator E2E
...
- `docs/reports/eidp-v436-stage6-evidence-draft.md`

Use the ZIP-embedded `docs/runbooks/eidp-operator-e2e-template.md` as the blank
recording form. Copy v436 package, SHA256, gate, and Windows execution values
...
```bash
shasum -a 256 -c dist/eidp-windows-v436.zip.sha256
if [ -f dist/eidp-windows-v436-transfer-manifest.json ]; then
  python -m json.tool dist/eidp-windows-v436-transfer-manifest.json >/dev/null
fi
uv run python scripts/run_non_windows_release_gates.py \
  dist/eidp-windows-v436.zip \
  --skip-full-unit \
  --allow-docs-only-stale-package \
  --json \
  --output logs/release-gate-v436-docs-only-stale-before-windows-transfer.json
```

Expected:

- `shasum` reports `dist/eidp-windows-v436.zip: OK`.
...
scp dist/eidp-windows-v436.zip dist/eidp-windows-v436.zip.sha256 win:C:/EIDP-staging/
```
...
- `dist/eidp-windows-v436.zip`
- `dist/eidp-windows-v436.zip.sha256`
...
Copy-Item -LiteralPath (Join-Path $Source "eidp-windows-v436.zip") -Destination "C:\EIDP-staging\eidp-windows-v436.zip"
Copy-Item -LiteralPath (Join-Path $Source "eidp-windows-v436.zip.sha256") -Destination "C:\EIDP-staging\eidp-windows-v436.zip.sha256"
Get-ChildItem C:\EIDP-staging\eidp-windows-v436.zip*
```
...
```powershell
$Expected = "d94ec5f4c735b7730d83299fc484fbcd71e5c0544a0f484ae5d900b4c46e218e"
$Zip = "C:\EIDP-staging\eidp-windows-v436.zip"
...
```powershell
certutil -hashfile C:\EIDP-staging\eidp-windows-v436.zip SHA256
```
...
```powershell
$Zip = "C:\EIDP-staging\eidp-windows-v436.zip"
$Dest = "C:\Users\cyo20\EIDP-v436-f56b2e4b"
if (Test-Path $Dest) {
...
```powershell
Set-Location "C:\Users\cyo20\EIDP-v436-f56b2e4b"
.\EIDP-setup.bat
```
...
- scheduled task points to this v436 extraction unless intentionally skipped
...
```bash
scp win:C:/Users/cyo20/EIDP-v436-f56b2e4b/logs/stage6-evidence-*.zip logs/
latest_bundle="$(ls -t logs/stage6-evidence-*.zip | head -n 1)"
test -n "$latest_bundle" || { echo "No logs/stage6-evidence-*.zip found"; exit 1; }
verify_json="logs/stage6-evidence-verify-mac-v436-$(date +%Y%m%d-%H%M%S).json"
...
- `C:\Users\cyo20\EIDP-v436-f56b2e4b\logs\stage6-evidence-*.zip`
- `C:\Users\cyo20\EIDP-v436-f56b2e4b\logs\stage6-evidence-verify-*.json`
- `C:\Users\cyo20\EIDP-v436-f56b2e4b\logs\run-*.log`
- `C:\Users\cyo20\EIDP-v436-f56b2e4b\data\output\last_run.json`
