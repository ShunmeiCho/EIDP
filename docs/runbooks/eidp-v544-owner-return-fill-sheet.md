# EIDP v544 Owner Return Fill Sheet

Date: 2026-06-21
Scope: v544 owner/operator return evidence only
...
3. the operator runs one real Stage 6 cycle on the v544 side-by-side lane; and
...
| E2E field | Value |
| --- | --- |
| EIDP package commit | `74325bc278c3e96052ef27e67cd554e426c87c60` |
| core ZIP ファイル名 | `eidp-windows-v544.zip` |
| core ZIP sha256 | `781da0a3c1a3f4ae80536c68de2971a1ae431a01c7eb2d58001de061f62df0c1` |
| transferred ZIP | `C:\EIDP-staging\eidp-windows-v544.zip` |
| Windows extract path | `%USERPROFILE%\EIDP-v544-74325bc-env0` |
| current active root to preserve | `%USERPROFILE%\EIDP-v527-69fe81f-env0` |
| non-Windows release gate | `logs/eidp-windows-v544-release-gates-20260621.json` -> `ok=true` |
| GitHub main CI | packaged source commit 74325bc: CI `27885705555` -> success; v544 evidence-docs commit 1b5b5e9: CI `27887120113` -> success; re-check current `main` before release approval |
| Windows validation evidence | `docs/reports/2026-06-21-v544-triage-helper-windows-canary.md` |
| OCR runtime evidence | v544 OCR runtime validation is not complete; required if OCR remains in v1.0 scope |
| setup validation evidence | `docs/reports/2026-06-21-v544-triage-helper-windows-canary.md` records after-setup validator `ok=true` |
| after-weekly validation evidence | `docs/reports/2026-06-21-v544-triage-helper-windows-canary.md` records after-weekly validator `ok=true` |
| bounded canary evidence | strict/Excel-ready `12/50 (24.0%)`, operator-reviewable `47/50 (94.0%)`, `ship_gate_status=below_gate` |
| Stage 6 evidence verify | `logs/win-v544-74325bc-canary/stage6-evidence-verify-20260621-080339.json` -> `ok=true` |
...
The v544 bounded canary is side-by-side smoke evidence only. It is not owner
real-cycle sign-off evidence.

## Short Owner Decision Form

Use `docs/runbooks/eidp-v544-release-summary.md` as the owner-facing one-page
summary and `docs/runbooks/eidp-v544-owner-signoff.md` as the short owner
decision form. Do not ask the owner to manually fill or reproduce the
engineering checklist.

For v544, current evidence supports `NOT_READY` only. `publication_lag`
...
- v544 setup validation evidence
- v544 active-task recovery JSON
...
Developer validation is run from current `main`, or from a v544+ package
carrying the same verifier integration, after the returned CSV is copied back
to the repo. The v544 package proves the verifier is available; it does not
prove the returned worksheet is complete.
...
uv run python scripts/verify_stage6_return.py \
  ...
  --owner-signoff docs/runbooks/eidp-v544-owner-signoff.md \
  --expected-package-sha256 781da0a3c1a3f4ae80536c68de2971a1ae431a01c7eb2d58001de061f62df0c1 \
  --expected-source-commit 74325bc278c3e96052ef27e67cd554e426c87c60 \
  --false-reject-evidence-zip logs/win-v544-74325bc-canary/stage6-evidence-20260620-230327.zip \
  --false-reject-review-csv docs/reports/2026-06-21-v544-false-reject-review-sheet.csv \
...
uv run python scripts/build_false_reject_audit.py \
  ...
  --sample-size 12 \
  --validate-review-csv docs/reports/2026-06-21-v544-false-reject-review-sheet.csv \
  --require-decisions
```
