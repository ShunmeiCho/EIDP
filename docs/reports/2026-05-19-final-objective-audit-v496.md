# Final Objective Audit - v496 Mac-side State
...
Mac package: `dist/eidp-windows-v496.zip`
...
v496 is the current Mac-side package/source-verified candidate, and PR #2 is
...
| --- | --- | --- |
| 47 prefecture official-list seeds | `logs/win-v496-stage6-v496-verify-windows-distribution-20260519.json`: `prefecture_seed_rows=47`, `prefecture_seed_downloadable=47`, `prefecture_seed_parser_supported=47` | PASS |
...
| Verifier-accepted mature-year proof JSON | `logs/mature-year-acquisition-proof-fy2025-release-exception-v496-20260519.json`: `ok=true`, basis `mature_year_retroactive_strict_target_pdf_and_operator_reviewable_acquisition`, FY2025 denominator `1000`, strict `60.0%`, Excel-ready `60.0%`, manual workload `20.2%` | PASS for release-exception proof input |
| PDF extraction stack | v496 package verifier passed; package includes the project wheel and runtime, with `wheel_count=84`, `entry_count=3104`, and `has_runtime=true` | PASS |
| Confidence `>= 0.70` gating | v496 non-Windows gate `logs/win-v496-stage6-v496-non-windows-release-gates-20260519.json` returned `ok=true`; validator/distribution subset passed with package/source freshness | PASS for tested code path |
...
| ZIP distribution and offline setup | `dist/eidp-windows-v496.zip`, SHA256 `cce8193244d68e41710195564eabf62cfabd22f93f12e8e1133f8daddda63273`; `BUILD_INFO.git_dirty=false`; Mac package verifier `ok=true` | PASS for Mac-side package, PENDING for Windows side-by-side |
| Browser UI on Windows | Latest Windows UI smoke remains v489: `logs/win-v489-stage6-v489-ui-smoke-20260519.json` returned `ok=true`, health `200/ok`, root `200`, stopped cleanly | PASS for v489, PENDING for v496 |
...
| Package/source freshness | v496 package was built from `6e3228934b3fd4ea1103475ebfc9cd9d6397089b`. `logs/win-v496-stage6-v496-non-windows-release-gates-20260519.json`: `package_source_check.ok=true`, `source_dirty=false`, `stale=false`. Any later docs-only status commit must be checked with `--allow-docs-only-stale-package` before merge/release. | PASS |
...
- v489 Windows setup/UI smoke proves a previous side-by-side package can start
  on the operator PC. It does not prove v496 Windows operability.
...
- A publication-lag release exception also requires a verifier-accepted
  mature-year proof JSON. v496 adds an explicit strict-gap-analysis input path
...
SSH/Windows access was intentionally avoided after the v493/v494/v496 audit because the
operator PC connection was at risk of dropping. The following Mac-side checks
were re-run against v493 without touching the Windows active lane. v496 package
...
1. Keep v1.0 blocked until FY2026/R8 public target PDFs become available, or
   ...
   evidence.
2. When SSH/operator PC access is stable, side-by-side validate v496 on Windows
...
4. If no exception is approved, do not promote owner sign-off as release-ready;
   keep v496 as Mac-side package evidence and v489 as Windows side-by-side
   evidence only.
