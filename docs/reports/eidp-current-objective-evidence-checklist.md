...
UI-health, bounded canary, and disk-retention lane. v454 remains the latest
Windows browser read-only navigation, R7 browser Excel generation/download,
default launcher, UI write/audit sandbox proof, and retained fallback package.
...
| Excel template export | v454 process-scoped FY2025 browser Excel smoke launched with `EIDP_TARGET_FISCAL_YEAR=2025`, rendered `④ Excel プレビュー` with `Excel出力可 2`, `Excel対象行 7177`, and `対象年度: 2025年度（令和7年度）`, generated workbook rows `採録状況=2418`, `対象比率=10024`, `学科別=9746`, `在籍のみ抜粋=9746`, exposed `Excel ダウンロード`, and downloaded `output/playwright/v454-r7-excel-smoke/eidp-master.xlsx`; local `openpyxl` verified sheets `採録状況`, `対象比率`, `学科別`, `在籍のみ抜粋` with dimensions `2419x10`, `10025x22`, `9748x83`, `9748x19`; Win-side checks reported both checked v454 `.env` locations missing, so the retroactive FY was not persisted. v442 remains historical support for the fuller R7 parity workbook, while historical v408 R7 CLI/browser exports and v437 FY2025/FY2024/FY2023 non-Windows retroactive matrix remain regression support. FY2026 export remains disabled on current setup evidence | Current v454 R7 browser Excel path proven; full operator-cycle Excel output still pending |
| ManualActionLog audits every operator action | v384 manual-entry, fiscal override, URL-candidate reject, and audit outbox browser smokes; source HEAD dedups audit outbox archives by matching filename stem for both default and custom outbox paths and ignores archive symlinks; v407 disposable UI sandbox flushed seven operator actions with `exported=7 already_present=0 failed=0` and `jsonl_exported_at_present=true` for all seven rows; v408 disposable UI sandbox repeated the audit path through `監査ログ`, showing `JSONL outbox 未送信=7`, `Outbox を flush` result `exported=7 already_present=0 failed=0`, and seven rows with `jsonl_exported_at_present=true` in direct DB verification; current v454 disposable UI write/audit sandbox rejected one seeded `URL候補レビュー` item and flushed two pending audit rows, with `logs/win-v454-stage6/v454-ui-write-sandbox-result-final.json` reporting `ok=true`, `pending_outbox=0`, exported `stage6_v454_ui_audit_flush_smoke` and `url_candidate_rejected` rows, no `SchoolSite` for the rejected URL, and real runtime DB marker counts all `0` | Current v454 proves URL-candidate reject + audit-outbox flush on a disposable DB; broader manual-entry/fiscal-override UI write coverage remains historical v408/v384; real operator one-cycle proof still missing |
| ZIP distribution, double-click setup, browser UI offline operation | v456 ZIP/SHA was transferred to `C:\EIDP-staging`, expanded to `C:\Users\cyo20\EIDP-v456-f33ffc0`, and `EIDP-setup.bat` completed with SQLite integrity, `school_fiscal_year_status_count=2418`, and `wheel_count=78`; independent `scripts\validate_install.bat --after-setup --json` returned `ok=true`. v456 Mac cleanup retained v456/v454/v453 and Win cleanup removed 48 old loose home ZIPs plus v453 staging/deploy artifacts, freeing about `8.91GB`; packaged disk health reported `warn_count=0` and `block_count=0`. v456 URL-only bootstrap downloaded/aggregated 47 prefecture seed artifacts, loaded `school_domain_overrides.csv` with `count=6`, and reported `school_override_inferred=6`; bounded `scripts\weekly_run.bat` exited `0` with `EIDP_TARGET_FISCAL_YEAR=2025`, methods including `school_domain_override`, `downloaded=2`, `new_document_ids=[1, 2]`, and `target_pdf_auto_yield_pct=40.0`; recovery check with expected action `C:\Users\cyo20\EIDP-v456-f33ffc0\scripts\weekly_run.bat` returned `ok=true` and `action_matches_expected=true`; refreshed evidence bundle `logs/win-v456-stage6/stage6-evidence-20260516-034752.zip` verified with all required labels and `entry_count=12`; direct Streamlit UI health smoke returned `_stcore/health=ok` and root HTTP `200` on `127.0.0.1:8501`, then cleanup left no listener. v454 remains historical browser-nav, R7 browser Excel, default-launcher, and UI write/audit sandbox proof. v408 remains historical copied-DB coverage for broader UI write paths. | Current v456 setup/bootstrap/validate/recovery/evidence/weekly launcher/school-domain-override/UI health/disk retention proven; v454 still covers browser nav/UI write/R7 Excel/default launcher; real operator one-cycle missing |
...
- Current v456 package/Windows setup lane: package snapshot
  ...
  `ok=true`. The v456 bundle
  `logs/win-v456-stage6/stage6-evidence-20260516-034752.zip` verified on Mac
  with `ok=true`, `manifest_missing_patterns=[]`, and all required labels. v456
...
- Current v454 package/Windows setup lane: package snapshot
  ...
  evidence-bundle verification with `weekly_run_logs`, UI health, browser read-only navigation through
  `output/playwright/v454-ui-smoke/`, process-scoped R7 Excel browser
  generation/download through `output/playwright/v454-r7-excel-smoke/`,
  UI write/audit sandbox proof through
  `output/playwright/v454-ui-write-sandbox/` and
  `logs/win-v454-stage6/v454-ui-write-sandbox-result-final.json`, default
  `EIDP-start.bat` launcher health over local and tunneled HTTP, and v454-current
  plus v453-fallback retention. v442 remains historical support for the fuller
  R7 parity workbook.
