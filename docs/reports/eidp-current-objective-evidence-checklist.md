...
`dist/eidp-windows-v408.zip` / code evidence base `f0c27158`. v426 is the
...
disconnected, so `docs/runbooks/eidp-v426-windows-transfer-checklist.md` now
...
v426 includes the current source/package lane plus a package-neutral reusable
...
protected-file guards in Stage 6 residual cleanup. Its
packaged `BUILD_INFO.json` records snapshot
`4ddc42b22cbf653eebefd6cc716c0bb0ed78aa20`.
...
- Current Mac/non-Windows release-gate proof: v426, package snapshot
  `4ddc42b22cbf653eebefd6cc716c0bb0ed78aa20`, SHA256
  `ac39b12304a0a611264334f5b106b998eb8a1b25018da2ae5c93c417256856dc`.
  `scripts/verify_windows_distribution.py dist/eidp-windows-v426.zip`
  ...
  `unit_full` returning `1573 passed`, validator/distribution tests returning `164 passed`,
  ...
  passing. The follow-up `logs/release-gate-v426-retroactive-matrix.json`
  matrix returned `ok=true` with `case_count=3` for FY2025, FY2024, and FY2023.
  The per-year outputs `logs/release-gate-v426-retroactive-fy2025-reference.json`,
  ...
  `_temp/non-windows-retroactive-fy2025-20260515-163006`,
  `_temp/non-windows-retroactive-fy2024-20260515-163126`, and
  `_temp/non-windows-retroactive-fy2023-20260515-163305`; all three produced
  the same four workbook row counts and zero business-value diffs against their
  stable references. v426 is not Windows-proven. v421 is explicitly superseded
  and must not be transferred because its package verifier rejected hard-coded
  v420 package/SHA fields in the packaged E2E template. v426 additionally
  verifies localhost launch, operator review lock, PDF annotation URI-safety,
  per-call Excel threshold, and protected residual-cleanup regressions.
...
- Current packaged source-code evidence base: `4ddc42b2`, incorporating the
...
Latest v426 Mac/non-Windows release-gate evidence, latest v408 setup/UI lane
...
- `uv run python scripts/build_windows_zip.py --skip-download --out-zip dist/eidp-windows-v426.zip --latest-alias`
  -> wrote `dist/eidp-windows-v426.zip`, refreshed `dist/eidp-windows.zip`,
  and wrote `dist/eidp-windows-v426.zip.sha256`.
- `uv run python scripts/run_non_windows_release_gates.py dist/eidp-windows-v426.zip --json --output logs/release-gate-v426-full.json`
  -> exited `0`: `unit_full` returned `1573 passed`, validator/distribution
...
lane from the latest Mac/non-Windows-clean package, v426. v408 remains the
...
First transfer and verify `dist/eidp-windows-v426.zip`:

```text
Package snapshot: 4ddc42b22cbf653eebefd6cc716c0bb0ed78aa20
Expected SHA256: ac39b12304a0a611264334f5b106b998eb8a1b25018da2ae5c93c417256856dc
Suggested extract path: C:\Users\cyo20\EIDP-v426-4ddc42b2
Checklist: docs/runbooks/eidp-v426-windows-transfer-checklist.md
```
...
back. In either case, complete the Stage 6 click-through against the real v426
