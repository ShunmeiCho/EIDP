...
- v523 owner-return verifier coverage audit is recorded in `docs/reports/2026-05-20-v523-owner-return-verifier-coverage-audit.md`: `verify_stage6_return.py` enforces last_run KPI consistency, Stage 6 evidence labels, selected E2E KPI rows, sign-off blocks, approved `publication_lag` records, and mature-year proof, but does not machine-enforce Excel proof or ManualActionLog / JSONL outbox consistency. A green return verifier is therefore necessary but not sufficient for owner-cycle acceptance.
