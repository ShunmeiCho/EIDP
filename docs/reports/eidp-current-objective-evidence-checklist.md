...
with the current v516/v502/v501 state.
...
- Current package candidate: `dist/eidp-windows-v516.zip`
- Package source commit from ZIP `BUILD_INFO.json`:
  `3b31eed865e57b0668345899f0de40001452b191`
- Package SHA256:
...
- Latest source/package discovery fix: v516 target-missing queue confirmed-target exclusion
- Latest FY2026/R8 Mac-side continuation canary:
  `docs/reports/2026-05-20-v515-sanko-child-overrides-package.md`
- Release verdict: blocked by FY2026/R8 strict yield, missing v516 Windows smoke,
...
| --- | --- | --- |
| 47 prefecture official-list seeds are packaged and usable | `logs/win-v516-stage6-v516-non-windows-release-gates-20260520.json`, result `package_verify` stdout: `prefecture_seed_rows=47`, `prefecture_seed_downloadable=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_school_rows_total=2148` | PASS |
| 1,700+ vocational-school scope | v502 Windows setup validator `logs/win-v502-stage6-v502-env0-validate-after-setup-20260520.json`: `.details.school_count=2418`, `.details.school_fiscal_year_status_count=2418`, `.details.sqlite_integrity_check="ok"`; v516 Windows setup pending | PASS via v502, v516 pending |
...
| PDF extraction stack is packaged | v516 package verifier stdout: `has_runtime=True`, `wheel_count=84`; v501 OCR runtime proof `logs/win-v501-stage6-v501-validate-ocr-runtime-20260520.json` is `ok=true` with Tesseract `5.4.0.20240606`, `jpn`, and `jpn_vert` | PASS for package, v516 Windows OCR runtime pending |
| Confidence `>= 0.70` gate exists | v516 full unit suite in release gate: `1892 passed`; confidence/export/review tests are covered by the unit suite | PASS for code contract, PARTIAL for production OCR corpus |
| `DepartmentYearly` and `SupportRecipient` append-only paths exist | v502 install validator confirms required tables including `department_yearly`, `support_recipient`, and `manual_action_log`; v516 unit suite is green | PASS for code/schema, PARTIAL for real operator workflow |
...
| PR merge and v1.0 tag are allowed | FY2026 strict proof, v516 Windows smoke, owner real cycle, and exception approval are incomplete | BLOCKED |
...
2. Complete v516 transfer/setup, OCR runtime, UI smoke, Excel smoke, Stage 6 evidence bundle,
