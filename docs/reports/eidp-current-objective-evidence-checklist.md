...
with the current v510/v502/v501 state.
...
- Current package candidate: `dist/eidp-windows-v510.zip`
- Package source commit from ZIP `BUILD_INFO.json`:
  `110bef2c24f2ef8722172ca2ddf27f81ae3a9813`
- Package SHA256:
  `50567f74722ff85fd04c0a73d562b6d0110322685c864bfa627e7ed41f076e4a`
- Latest complete Windows side-by-side smoke: v501
- Latest partial Windows side-by-side setup/canary: v502
- Latest source/package audit-surface fix: v510 school alias approval audit
- Release verdict: blocked by FY2026/R8 strict yield, missing v510 Windows smoke,
...
| --- | --- | --- |
| 47 prefecture official-list seeds are packaged and usable | `logs/win-v510-stage6-v510-non-windows-release-gates-20260520.json`, result `package_verify` stdout: `prefecture_seed_rows=47`, `prefecture_seed_downloadable=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_school_rows_total=2148` | PASS |
| 1,700+ vocational-school scope | v502 Windows setup validator `logs/win-v502-stage6-v502-env0-validate-after-setup-20260520.json`: `.details.school_count=2418`, `.details.school_fiscal_year_status_count=2418`, `.details.sqlite_integrity_check="ok"`; v510 Windows setup pending | PASS via v502, v510 pending |
...
| PDF extraction stack is packaged | v510 package verifier stdout: `has_runtime=True`, `wheel_count=84`; v501 OCR runtime proof `logs/win-v501-stage6-v501-validate-ocr-runtime-20260520.json` is `ok=true` with Tesseract `5.4.0.20240606`, `jpn`, and `jpn_vert` | PASS for package, v510 Windows OCR runtime pending |
| Confidence `>= 0.70` gate exists | v510 full unit suite in release gate: `1888 passed`; confidence/export/review tests are covered by the unit suite | PASS for code contract, PARTIAL for production OCR corpus |
| `DepartmentYearly` and `SupportRecipient` append-only paths exist | v502 install validator confirms required tables including `department_yearly`, `support_recipient`, and `manual_action_log`; v510 unit suite is green | PASS for code/schema, PARTIAL for real operator workflow |
...
| PR merge and v1.0 tag are allowed | FY2026 strict proof, v510 Windows smoke, owner real cycle, and exception approval are incomplete | BLOCKED |
...
2. Complete v510 transfer/setup, OCR runtime, UI smoke, Excel smoke, Stage 6 evidence bundle,
