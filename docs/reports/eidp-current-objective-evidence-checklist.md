...
`dist/eidp-windows-v408.zip` / code evidence base `f0c27158`. v429 is the
...
disconnected, so `docs/runbooks/eidp-v429-windows-transfer-checklist.md` now
...
v429 includes the current source/package lane plus a package-neutral reusable
operator E2E template. It also carries local-only Streamlit launch, app-lock
acquisition around school-code and URL-candidate review writes, safe `http(s)`
filtering for PDF annotation links, per-call Excel confidence thresholds, and
protected-file guards in Stage 6 residual cleanup. It also surfaces future or
otherwise out-of-cap ingest fiscal-year annotations as `invalid_fiscal_year`
stats/evidence rather than a generic no-year parse failure. Its
packaged `BUILD_INFO.json` records snapshot
`bdf7f4215546f9945a82be36b3fb813a75dac5e3`.
...
- Current Mac/non-Windows release-gate proof: v429, package snapshot
  `bdf7f4215546f9945a82be36b3fb813a75dac5e3`, SHA256
  `b3acf94ec6dc3f7cd69496972a75e5eec4915e6adc8fdda7d49fb2d1e2c2de45`.
  `scripts/verify_windows_distribution.py dist/eidp-windows-v429.zip`
  ...
  `unit_full` returning `1579 passed`, validator/distribution tests returning `164 passed`,
  ...
  passing. The follow-up `logs/release-gate-v429-retroactive-matrix.json`
  matrix returned `ok=true` with `case_count=3` for FY2025, FY2024, and FY2023.
  The per-year outputs `logs/release-gate-v429-retroactive-fy2025-reference.json`,
  ...
  `_temp/non-windows-retroactive-fy2025-20260515-171207`,
  `_temp/non-windows-retroactive-fy2024-20260515-171252`, and
  `_temp/non-windows-retroactive-fy2023-20260515-171335`; all three produced
  the same four workbook row counts and zero business-value diffs against their
  stable references. v429 is not Windows-proven. v421 is explicitly superseded
  and must not be transferred because its package verifier rejected hard-coded
  v420 package/SHA fields in the packaged E2E template. v429 additionally
  verifies localhost launch, operator review lock, PDF annotation URI-safety,
  per-call Excel threshold, protected residual-cleanup, and fiscal-year
  override collateral-demotion audit regressions. v429 also surfaces
  future/out-of-cap ingest fiscal-year annotations through `invalid_fiscal_year`
  stats/evidence. v429 also widens the CLI
  write-lock AST contract to cover every `cli_*.py` command module and
  attribute-form write helper calls.
...
- Current packaged source-code evidence base: `bdf7f421`, incorporating the
  ...
  fiscal-year annotations, plus unit-test isolation for Streamlit AppTest's fake `__main__`
...
Latest v429 Mac/non-Windows release-gate evidence, latest v408 setup/UI lane
...
- `uv run python scripts/build_windows_zip.py --skip-download --out-zip dist/eidp-windows-v429.zip --latest-alias`
  -> wrote `dist/eidp-windows-v429.zip`, refreshed `dist/eidp-windows.zip`,
  and wrote `dist/eidp-windows-v429.zip.sha256`.
- `uv run python scripts/run_non_windows_release_gates.py dist/eidp-windows-v429.zip --json --output logs/release-gate-v429-full.json`
  -> exited `0`: `unit_full` returned `1579 passed`, validator/distribution
...
lane from the latest Mac/non-Windows-clean package, v429. v408 remains the
...
First transfer and verify `dist/eidp-windows-v429.zip`:

```text
Package snapshot: bdf7f4215546f9945a82be36b3fb813a75dac5e3
Expected SHA256: b3acf94ec6dc3f7cd69496972a75e5eec4915e6adc8fdda7d49fb2d1e2c2de45
Suggested extract path: C:\Users\cyo20\EIDP-v429-bdf7f421
Checklist: docs/runbooks/eidp-v429-windows-transfer-checklist.md
```
...
back. In either case, complete the Stage 6 click-through against the real v429
