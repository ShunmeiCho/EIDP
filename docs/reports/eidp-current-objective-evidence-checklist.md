...
with the current v502/v501 state.
...
- Current package candidate: `dist/eidp-windows-v502.zip`
- Package source commit from ZIP `BUILD_INFO.json`:
  `dd1524c48240890a8260795b54259342d7648867`
- Package SHA256:
...
- Release verdict: blocked by FY2026/R8 strict yield, missing v502 full smoke,
...
| --- | --- | --- |
| 47 prefecture official-list seeds are packaged and usable | `logs/win-v502-stage6-v502-non-windows-release-gates-20260520.json`, result `package_verify` stdout: `prefecture_seed_rows=47`, `prefecture_seed_downloadable=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_school_rows_total=2148` | PASS |
| 1,700+ vocational-school scope | `logs/win-v502-stage6-v502-env0-validate-after-setup-20260520.json`: `.details.school_count=2418`, `.details.school_fiscal_year_status_count=2418`, `.details.sqlite_integrity_check="ok"` | PASS |
...
| PDF extraction stack is packaged | v502 package verifier stdout: `has_runtime=True`, `wheel_count=84`; v501 OCR runtime proof `logs/win-v501-stage6-v501-validate-ocr-runtime-20260520.json` is `ok=true` with Tesseract `5.4.0.20240606`, `jpn`, and `jpn_vert` | PASS for package, v502 Windows OCR runtime pending |
| Confidence `>= 0.70` gate exists | v502 full unit suite in release gate: `1880 passed`; confidence/export/review tests are covered by the unit suite | PASS for code contract, PARTIAL for production OCR corpus |
| `DepartmentYearly` and `SupportRecipient` append-only paths exist | v502 install validator confirms required tables including `department_yearly`, `support_recipient`, and `manual_action_log`; unit suite is green | PASS for code/schema, PARTIAL for real operator workflow |
...
| Operator actions are auditable in `ManualActionLog` | v502 install validator confirms the table; unit/UI historical evidence covers audit writes. Current owner real-cycle audit counts and sign-off are missing | PARTIAL |
...
| PR merge and v1.0 tag are allowed | FY2026 strict proof, v502 full smoke, owner real cycle, and exception approval are incomplete | BLOCKED |
...
- `git status --short --branch` was clean before this documentation refresh.
...
2. Complete v502 OCR runtime, UI smoke, Excel smoke, Stage 6 evidence bundle,
