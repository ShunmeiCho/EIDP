...
Windows, packaged v464 disk health is non-blocking, and v464 now carries the
latest process-scoped FY2025/R7 browser Excel preview/download proof. v463
remains the latest
side-by-side target-FY override canary package, and v462 remains the
...
evidence-bundle-proven, default-launcher-proven, bounded-weekly-smoked, and UI
write/audit sandbox support package. v456 and v454 remain historical support for broader UI
...
`action_matches_expected=true`. v464 read-only UI smoke also returned
Windows-local and tunneled health `ok`, captured five screenshots under
`output/playwright/v464-ui-smoke/`, and recorded `nav_all_clicked=true`,
...
unmeasured and sign-off/template rows were blank. Packaged v464 disk health
returned `ok=true`, `warn_count=0`, and `block_count=0`. A process-scoped v464
FY2025/R7 browser Excel smoke then launched the same side-by-side package with
`EIDP_TARGET_FISCAL_YEAR=2025` on Windows `127.0.0.1:8509`, reached it through
Mac tunnel `127.0.0.1:18509`, rendered `④ Excel プレビュー`, generated the
preview workbook, clicked `Excel ダウンロード`, and wrote
`output/playwright/v464-r7-excel-smoke/eidp_master.xlsx` with SHA256
`aff3dea57af4c6d96d8859e52748f8cecefb4e593f5da74b4f68646175937685`.
`output/playwright/v464-r7-excel-smoke/summary.json` returned `ok=true`; the
four workbook sheets matched the UI data-row counts
`採録状況=2418`, `対象比率=10022`, `学科別=9719`, and `在籍のみ抜粋=9719`.
Cleanup removed the remote `8509` listener and local `18509` tunnel, and the
scheduled task still pointed to the v460 runner. v463 remains the latest
...
v460 at `C:\Users\cyo20\EIDP-v460-01e4427`: v464 side-by-side transfer/setup/UI
health passed, and v463 side-by-side transfer/setup/FY-override canary passed,
but neither v464 nor v463 has been weekly-smoked, evidence-bundled, or made the
...
| Excel template export | v464 process-scoped FY2025 browser Excel smoke launched with `EIDP_TARGET_FISCAL_YEAR=2025`, rendered `④ Excel プレビュー` with `2025年度（令和7年度）`, clicked `プレビュー workbook を生成`, exposed `Excel ダウンロード`, and downloaded `output/playwright/v464-r7-excel-smoke/eidp_master.xlsx`; local `openpyxl` verified sheets `採録状況`, `対象比率`, `学科別`, `在籍のみ抜粋` with dimensions `2419x10`, `10023x22`, `9721x83`, `9721x19`, matching UI data-row counts `2418`, `10022`, `9719`, and `9719`; `summary.json` records `ok=true`, `write_actions_invoked=false`, and `weekly_invoked=false`. v460 adds Mac-side hardening that serializes preview bytes and drops the workbook handle from Streamlit session state, but no v460 browser Excel run has been executed yet. v459, v456, v454, and v442 remain historical browser support, while historical v408 R7 CLI/browser exports and v437 FY2025/FY2024/FY2023 non-Windows retroactive matrix remain regression support. Fresh v463 Mac retroactive matrix proof now passes FY2025/FY2024/FY2023 against references regenerated from the frozen v459 package, with zero missing/extra rows and zero differing fields. FY2026 export remains disabled on current setup evidence | Latest browser Excel proof is v464 R7 support; latest Mac algorithm regression proof is v463 three-year matrix; v460 full operator-cycle Excel output still pending |
...
| Latest side-by-side UI health | v466 Streamlit was started directly on Windows `127.0.0.1:8511`; Windows-local health and Mac tunnel `127.0.0.1:18511 -> 127.0.0.1:8511` both returned `ok`. Playwright clicked only read-only quick navigation buttons for `① 学校別タスク`, `② PDF確認・手入力`, `③ 年度判定・修正`, `④ Excel プレビュー`, and `⑤ 設定（年度・OCR・API）`; `output/playwright/v466-ui-smoke/summary.json` reports `ok=true`, `has_v466_build=true`, `has_target_fiscal_year=true`, `target_fiscal_year_text=2026年度（令和8年度）`, `has_error_traceback=false`, `nav_all_clicked=true`, `write_actions_invoked=false`, and `weekly_invoked=false`. Screenshots `v466-ui-smoke-00-home.png` through `v466-ui-smoke-05-settings.png` were captured, cleanup removed both listeners, and a fresh scheduled-task XML check still pointed to v460. | v466 Windows side-by-side UI-health/read-only nav proven; not a real operator cycle |
| Latest side-by-side R7 browser Excel | v464 Streamlit was started directly on Windows `127.0.0.1:8509` with process-scoped `EIDP_TARGET_FISCAL_YEAR=2025`; Windows-local health and Mac tunnel `127.0.0.1:18509 -> 127.0.0.1:8509` both returned `ok`. Playwright rendered `④ Excel プレビュー`, generated the preview workbook, clicked `Excel ダウンロード`, saved `output/playwright/v464-r7-excel-smoke/eidp_master.xlsx`, and `openpyxl` verified the four expected sheets and data-row counts. Cleanup removed both listeners and reconfirmed the scheduled task still points to v460. | v464 Windows side-by-side FY2025/R7 Excel preview/download proven; not a real operator cycle and not FY2026/R8 yield evidence |
...
| Latest side-by-side disk health | v464 packaged `scripts\disk_health_check.py --profile operator-win --json` returned `ok=true`, `warn_count=0`, `block_count=0`; `logs/win-v464-stage6/disk-health-20260517-operator-win.json` records app root `844.1MiB`, `data/pdfs=0B`, `data/output=0B`, `logs=12.8KiB`, and missing protected audit outbox because no v464 operator action has run. | v464 side-by-side disk health non-blocking |
...
- `uv run mypy src` is now a usable source-wide gate for the tracked source
  ...
  rolling FY yield gate. The v459 browser navigation and v459 UI write/audit
  sandbox, plus the v464 R7 browser Excel proof, are real browser evidence, but
  they are still Codex-driven validation, not the owner/operator real-cycle
  sign-off.
