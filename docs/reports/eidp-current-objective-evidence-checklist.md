...
| ZIP distribution, double-click setup, browser UI offline operation | v407 transfer, SHA match, setup completion, SQLite integrity, CLI smoke, Streamlit health, `18501 -> 8501` tunnel health, verifier-accepted diagnostic bundle `logs\stage6-evidence-20260514-174426.zip` carrying the R7 browser Excel proof without Excel exports, and seeded browser write sandbox proof; v397 browser read-only navigation retained as historical support | Current v407 setup/service, diagnostic bundle, and sandbox browser-write proven; real operator one-cycle missing |
