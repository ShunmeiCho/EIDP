...
`dist/eidp-windows-v408.zip` / code evidence base `f0c27158`. v433 is the
...
disconnected, so `docs/runbooks/eidp-v433-windows-transfer-checklist.md` now
...
v433 includes the current source/package lane plus a package-neutral reusable
...
`9497b2c7eac95131bfa4b97dd0f1d4f5d2a7b1d1`.
...
- Current Mac/non-Windows release-gate proof: v433, package snapshot
  `9497b2c7eac95131bfa4b97dd0f1d4f5d2a7b1d1`, SHA256
  `3b07021ce0ff11874a3fb985e4ea1278cb81c54f866de572645c22721fd880c5`.
  `scripts/verify_windows_distribution.py dist/eidp-windows-v433.zip`
  ...
  `unit_full` returning `1584 passed`, validator/distribution tests returning `164 passed`,
  ...
  passing. The follow-up `logs/release-gate-v433-retroactive-matrix.json`
  matrix returned `ok=true` with `case_count=3` for FY2025, FY2024, and FY2023.
  The per-year outputs `logs/release-gate-v433-retroactive-fy2025-reference.json`,
  ...
  `_temp/non-windows-retroactive-fy2025-20260515-174330`,
  `_temp/non-windows-retroactive-fy2024-20260515-174415`, and
  `_temp/non-windows-retroactive-fy2023-20260515-174501`; all three produced
  the same four workbook row counts and zero business-value diffs against their
  stable references. v433 is not Windows-proven. v421 is explicitly superseded
  and must not be transferred because its package verifier rejected hard-coded
  v420 package/SHA fields in the packaged E2E template. v433 additionally
  ...
  collateral-demotion audit regressions. v433 also surfaces future/out-of-cap
  ingest fiscal-year annotations through `invalid_fiscal_year` stats/evidence,
  locks manual-entry `DepartmentYearly` revision reads before append, and locks
  school URL crawl-evidence JSONL appends. v433 also widens the CLI write-lock
...
- Current packaged source-code evidence base: `9497b2c7`, incorporating the
...
Latest v433 Mac/non-Windows release-gate evidence, latest v408 setup/UI lane
...
- `uv run python scripts/build_windows_zip.py --skip-download --out-zip dist/eidp-windows-v433.zip --latest-alias`
  -> wrote `dist/eidp-windows-v433.zip`, refreshed `dist/eidp-windows.zip`,
  and wrote `dist/eidp-windows-v433.zip.sha256`.
- `uv run python scripts/run_non_windows_release_gates.py dist/eidp-windows-v433.zip --json --output logs/release-gate-v433-full.json`
  -> exited `0`: `unit_full` returned `1584 passed`, validator/distribution
...
lane from the latest Mac/non-Windows-clean package, v433. v408 remains the
...
First transfer and verify `dist/eidp-windows-v433.zip`:

```text
Package snapshot: 9497b2c7eac95131bfa4b97dd0f1d4f5d2a7b1d1
Expected SHA256: 3b07021ce0ff11874a3fb985e4ea1278cb81c54f866de572645c22721fd880c5
Suggested extract path: C:\Users\cyo20\EIDP-v433-9497b2c7
Checklist: docs/runbooks/eidp-v433-windows-transfer-checklist.md
```
...
back. In either case, complete the Stage 6 click-through against the real v433
