...
`dist/eidp-windows-v408.zip` / code evidence base `f0c27158`. v430 is the
...
disconnected, so `docs/runbooks/eidp-v430-windows-transfer-checklist.md` now
...
v430 includes the current source/package lane plus a package-neutral reusable
...
stats/evidence rather than a generic no-year parse failure, and locks
manual-entry `DepartmentYearly` revision reads before appending replacement
current rows. Its
packaged `BUILD_INFO.json` records snapshot
`30913abb3c6038a259d65ff211638e5b55f256da`.
...
- Current Mac/non-Windows release-gate proof: v430, package snapshot
  `30913abb3c6038a259d65ff211638e5b55f256da`, SHA256
  `de68602f2d824f584e9e9d846c0df65858f7d12a7f773a44f0f5a05806a4d554`.
  `scripts/verify_windows_distribution.py dist/eidp-windows-v430.zip`
  ...
  `unit_full` returning `1580 passed`, validator/distribution tests returning `164 passed`,
  ...
  passing. The follow-up `logs/release-gate-v430-retroactive-matrix.json`
  matrix returned `ok=true` with `case_count=3` for FY2025, FY2024, and FY2023.
  The per-year outputs `logs/release-gate-v430-retroactive-fy2025-reference.json`,
  ...
  `_temp/non-windows-retroactive-fy2025-20260515-172037`,
  `_temp/non-windows-retroactive-fy2024-20260515-172125`, and
  `_temp/non-windows-retroactive-fy2023-20260515-172210`; all three produced
  the same four workbook row counts and zero business-value diffs against their
  stable references. v430 is not Windows-proven. v421 is explicitly superseded
  and must not be transferred because its package verifier rejected hard-coded
  v420 package/SHA fields in the packaged E2E template. v430 additionally
  ...
  override collateral-demotion audit regressions. v430 also surfaces
  future/out-of-cap ingest fiscal-year annotations through `invalid_fiscal_year`
  stats/evidence and locks manual-entry `DepartmentYearly` revision reads before
  append. v430 also widens the CLI
...
- Current packaged source-code evidence base: `30913abb`, incorporating the
  ...
  fiscal-year annotations and manual-entry row-lock coverage for
  `DepartmentYearly` append-only revision writes, plus unit-test isolation for Streamlit AppTest's fake `__main__`
...
Latest v430 Mac/non-Windows release-gate evidence, latest v408 setup/UI lane
...
- `uv run python scripts/build_windows_zip.py --skip-download --out-zip dist/eidp-windows-v430.zip --latest-alias`
  -> wrote `dist/eidp-windows-v430.zip`, refreshed `dist/eidp-windows.zip`,
  and wrote `dist/eidp-windows-v430.zip.sha256`.
- `uv run python scripts/run_non_windows_release_gates.py dist/eidp-windows-v430.zip --json --output logs/release-gate-v430-full.json`
  -> exited `0`: `unit_full` returned `1580 passed`, validator/distribution
...
lane from the latest Mac/non-Windows-clean package, v430. v408 remains the
...
First transfer and verify `dist/eidp-windows-v430.zip`:

```text
Package snapshot: 30913abb3c6038a259d65ff211638e5b55f256da
...
Checklist: docs/runbooks/eidp-v430-windows-transfer-checklist.md
```
...
back. In either case, complete the Stage 6 click-through against the real v430
