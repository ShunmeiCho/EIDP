...
| Install validator after weekly | pass | `scripts\validate_install.bat --after-setup --after-weekly --json` returned `ok=true`, `errors=[]`, `warnings=[]`, `sqlite_integrity_check=ok`, `last_run_status=success`, `sqlite_target_fy=2025`, `sqlite_target_fy_target_pdf_school_count=2`, and `sqlite_target_fy_operator_reviewable_school_count=5`; captured as `logs/win-v454-stage6/v454-validate-install-after-weekly.json`. |
