...
Latest audited Windows package commit: `fcbeed0b853234c756737473e7e935492eeeeccf` (`eidp-windows-v79.zip`)
...
| Windows operator delivery | `dist/eidp-windows.zip` rebuilt as v79 at commit `fcbeed0`, verifier `ok=true`, `git_dirty=false`, SHA256 `029af5c1d4540ea59b05dc4b006dbfc65884a232e229ef7a1138c473c87f3007`, wheelhouse 82 wheels. Latest Mac smoke confirms Streamlit starts, settings page imports, and Settings navigation has zero exceptions against an isolated SQLite DB. Remote Windows smoke is still only proven through v73: SHA256 match, clean setup exit code 0, `school_count=2418`, `school_fiscal_year_status_count=2418`, official-index yield, and bounded Step 3 cache/prefilter behavior. | Latest package is locally verified and ready for Windows transfer. Full Windows PDF crawl/ingest and UI click-through are still not complete on v79. |
...
- `uv run python scripts/verify_windows_distribution.py dist/eidp-windows.zip --json` on v79 → `ok=true`, `git_commit=fcbeed0b853234c756737473e7e935492eeeeccf`, `git_dirty=false`, `sha256=029af5c1d4540ea59b05dc4b006dbfc65884a232e229ef7a1138c473c87f3007`, `prefecture_seed_rows=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_downloadable=47`, no warnings
...
1. Validate target-year PDF yield with a full real bootstrap run.
   ...
   search counts.
2. Run latest v79 ZIP on Windows and verify:
