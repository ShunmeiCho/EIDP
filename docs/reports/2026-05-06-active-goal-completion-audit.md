...
Latest audited Windows package commit: `8969ceb3f6325a6c71a6ccd63a7fcb3164ec1291` (`eidp-windows-v214.zip`)
...
| Windows operator delivery | `dist/eidp-windows-v214.zip` rebuilt at commit `8969ceb3f6325a6c71a6ccd63a7fcb3164ec1291`, verifier `OK core`, `git_dirty=false`, SHA256 `ef1c88b528cc947a3b50dfee60376e83f8f1a5aaad9d08560adcfd5a32e4d3a4`, wheelhouse 78 wheels, 47 prefecture seed rows/parser registrations/downloadable artifact URLs, `prefecture_seed_school_rows_total=2148`, 16 packaged discovery gold-set entries, packaged OCR runtime contract checks for `src/eidp/ocr/tesseract.py` / `src/eidp/ocr/availability.py`, packaged SQLite bootstrap data-loss guards for orphaned WAL sidecars, `PRAGMA integrity_check`, additive `department_change` columns, and `uq_document_file_hash`, and packaged operator-action audit gates for school-code review, URL-candidate review, department-alias approval/void, manual entry, and fiscal-year override. The latest alias `dist/eidp-windows.zip` has the same SHA256. `dist/eidp-playwright-addon-windows-v106.zip` verifies with SHA256 `f6fe0cd095c337a81a870decb7a18e9d1f40044dd1567b017d92eda3aae1e8e8`, `entry_count=637`, and `manifest_files=636`. Remote Windows v154 smoke on a fresh `C:\EIDP-v154-26d18a9` extraction remains the latest real Windows E2E proof: setup success, `school_count=2418`, `school_fiscal_year_status_count=2418`, standalone validator `ok=true`, Saitama official-index apply (`matched=51`, `added=51`), school `757` packaged discovery accepts `study_support_system.pdf` with `year_evidence=prefecture_index_current_year`, school `757` packaged ingest writes the FY2026 row to the existing `医療` / `第一学科` Department with `extraction_confidence=0.94` and `is_current=True`, and Excel export contains one row for the school in both `学科別` and `在籍のみ抜粋`. v150 remains the packaged proof for school `95` target-PDF-to-Excel. | Latest v214 package verifier and extracted-install smoke are green on macOS. Latest real Windows extraction/setup/Saitama official-index/school-757 discovery/ingest/status/export smoke is still v154 because current Windows SSH is unavailable. Automation yield remains below the 60-70% strict target-FY ship gate; execution-button UI E2E and broader real-workload yield remain incomplete. |
...
- v214 Windows ZIP artifacts were rebuilt locally at commit
  `8969ceb3f6325a6c71a6ccd63a7fcb3164ec1291`: `dist/eidp-windows-v214.zip`
  and latest alias `dist/eidp-windows.zip` both have SHA256
  `ef1c88b528cc947a3b50dfee60376e83f8f1a5aaad9d08560adcfd5a32e4d3a4`.
  `uv run python scripts/verify_windows_distribution.py
  dist/eidp-windows-v214.zip --json` returned `ok=true`, `errors=[]`, and
  `warnings=[]`, with 3,026 ZIP entries and BUILD_INFO
  `git_dirty=false`. An extracted-install check at `/tmp/eidp-v214-extract`
