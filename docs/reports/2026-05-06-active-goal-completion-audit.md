...
Latest audited Windows package commit: `8bceae2e725d1d49fa57c9fcb3da341e5fe38106` (`eidp-windows-v91.zip`)
...
| Windows operator delivery | `dist/eidp-windows-v91.zip` rebuilt at commit `8bceae2`, verifier `ok=true`, `git_dirty=false`, SHA256 `3e4452a94cf0a5ad6a0b92e4ccb4197fda545fbeddeb5b9af3b3e2ffd66394d7`, wheelhouse 82 wheels. Latest package includes the current operator runbook, settings page, UI-first weekly rediscovery guidance, browser-proven non-transparent action buttons, bootstrap progress rejection breakdowns, URL-search provider/mode/limit visibility and package identity visibility on the task board, PDF確認 action-lane filtering, school-task source-chain CSV export, one-click `EIDP-diagnose.bat` diagnostics collection, 20-second auto-refresh while initial bootstrap is running, automatic recovery of stale Windows setup locks older than two hours, hardened ZIP/extracted-install gates for every navigated Streamlit route module, packaged runbook guidance that makes the package identity line the first screenshot check, and install-validator build identity fields for diagnostics-only feedback. Latest Mac smoke confirms Streamlit starts, settings page imports, Settings navigation has zero exceptions, and Chrome headless rendering shows `初回URL/PDF取得を開始` as a real styled button. Remote Windows smoke is still only proven through v73: SHA256 match, clean setup exit code 0, `school_count=2418`, `school_fiscal_year_status_count=2418`, official-index yield, and bounded Step 3 cache/prefilter behavior. | Latest package is locally verified and ready for Windows transfer. Full Windows PDF crawl/ingest and UI click-through are still not complete on v91. |
...
- `uv run pytest -q` → `834 passed, 5 warnings`
...
- `uv run python scripts/verify_windows_distribution.py dist/eidp-windows-v91.zip` → `OK core`, `git_commit=8bceae2e725d1d49fa57c9fcb3da341e5fe38106`, `git_dirty=false`, `sha256=3e4452a94cf0a5ad6a0b92e4ccb4197fda545fbeddeb5b9af3b3e2ffd66394d7`, `entry_count=2994`, `wheel_count=82`, `prefecture_seed_rows=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_downloadable=47`
- `uv run python scripts/verify_windows_distribution.py dist/eidp-windows.zip` → `OK core`, `git_commit=8bceae2e725d1d49fa57c9fcb3da341e5fe38106`, `git_dirty=false`, `sha256=3e4452a94cf0a5ad6a0b92e4ccb4197fda545fbeddeb5b9af3b3e2ffd66394d7`, `entry_count=2994`, `wheel_count=82`, `prefecture_seed_rows=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_downloadable=47`
- Extracted v91 ZIP smoke (`_temp/v91-extract-yzoXS3`) using the packaged `scripts/validate_windows_install.py` → `OK install`, `build_commit=8bceae2e725d1d49fa57c9fcb3da341e5fe38106`, `build_branch=sprint8-handoff-finalize`, `build_dirty=false`, `master_xlsx_present=True`, `wheel_count=82`
...
2. Run latest v91 ZIP on Windows and verify:
