...
Latest audited Windows package commit: `8c6ef016366c1a37e9fa335ca4bdb07119d3ac7e` (`eidp-windows-v212.zip`)
...
| Windows operator delivery | `dist/eidp-windows-v212.zip` rebuilt at commit `8c6ef016366c1a37e9fa335ca4bdb07119d3ac7e`, verifier `OK core`, `git_dirty=false`, SHA256 `37d3d0e460f1a1654f8aaeef59a3d3b6bdc00c9c0a9460a8806ae86ae04e8853`, wheelhouse 78 wheels, 47 prefecture seed rows/parser registrations/downloadable artifact URLs, `prefecture_seed_school_rows_total=2148`, 16 packaged discovery gold-set entries, and packaged OCR runtime contract checks for `src/eidp/ocr/tesseract.py` / `src/eidp/ocr/availability.py`. The latest alias `dist/eidp-windows.zip` has the same SHA256. `dist/eidp-playwright-addon-windows-v106.zip` verifies with SHA256 `f6fe0cd095c337a81a870decb7a18e9d1f40044dd1567b017d92eda3aae1e8e8`, `entry_count=637`, and `manifest_files=636`. Remote Windows v154 smoke on a fresh `C:\EIDP-v154-26d18a9` extraction remains the latest real Windows E2E proof: setup success, `school_count=2418`, `school_fiscal_year_status_count=2418`, standalone validator `ok=true`, Saitama official-index apply (`matched=51`, `added=51`), school `757` packaged discovery accepts `study_support_system.pdf` with `year_evidence=prefecture_index_current_year`, school `757` packaged ingest writes the FY2026 row to the existing `医療` / `第一学科` Department with `extraction_confidence=0.94` and `is_current=True`, and Excel export contains one row for the school in both `学科別` and `在籍のみ抜粋`. v150 remains the packaged proof for school `95` target-PDF-to-Excel. | Latest v212 package verifier and extracted-install smoke are green on macOS. Latest real Windows extraction/setup/Saitama official-index/school-757 discovery/ingest/status/export smoke is still v154 because current Windows SSH is unavailable. Automation yield remains below the 60-70% strict target-FY ship gate; execution-button UI E2E and broader real-workload yield remain incomplete. |
...
- 2026-05-11 v212 local package refresh →
  commit `8c6ef016366c1a37e9fa335ca4bdb07119d3ac7e` adds a release verifier
  gate for the packaged OCR runtime/availability contract. The core ZIP must
  now contain `src/eidp/ocr/tesseract.py` and `src/eidp/ocr/availability.py`
  with the add-on path, Japanese tessdata, Tesseract subprocess wrapper, and
  operator-facing OCR availability checks intact. Verification:
  `uv run pytest tests/unit/test_windows_distribution_verifier.py
  tests/unit/test_ocr_addon_packaging.py
  tests/unit/test_ocr_tesseract_wrapper.py
  tests/unit/test_ocr_availability.py -q` → `87 passed`;
  ...
  tests/unit -q` → `1138 passed, 5 warnings`.
- v212 Windows ZIP artifacts were rebuilt locally at commit
  `8c6ef016366c1a37e9fa335ca4bdb07119d3ac7e`: `dist/eidp-windows-v212.zip`
  and latest alias `dist/eidp-windows.zip` both have SHA256
  `37d3d0e460f1a1654f8aaeef59a3d3b6bdc00c9c0a9460a8806ae86ae04e8853`.
  `uv run python scripts/verify_windows_distribution.py
  dist/eidp-windows-v212.zip --json` returned `ok=true`, `errors=[]`, and
  `warnings=[]`, with 3,026 ZIP entries and BUILD_INFO
  `git_dirty=false`. An extracted-install check at `/tmp/eidp-v212-extract`
  returned `ok=true`, `errors=[]`, `warnings=[]`, `master_xlsx_present=true`,
  and `wheel_count=78`.
...
The active goal is **not complete**. v154 is the current locally and
...
same-origin 404 portion of that fetch/navigation bucket, v141 turns TLS
...
v145 keeps the few target forms without usable year evidence visible as
`年度未確認候補` / `PDF確認` operator work instead of generic no-target rows.
v146 proves the first strict FY2026 Saitama target PDF can run through the
packaged Windows PDF discovery, ingest, status rebuild, and Excel export path
...
`764` diagnostic replay. v152 supersedes v151 by packaging the official-index
trusted year-evidence rule, and the Windows package smoke proves school `757`
...
`医療`. v154 then parses the sparse graduation row, raising the school `757`
FY2026 yearly confidence to `0.94`, marking it current, rebuilding the status
row as `excel_ready=True`, and proving Excel export contains the school in both
business sheets. The main remaining blockers are target-year yield/policy,
broader extraction/OCR improvement for other accepted PDFs, real operator UI
