...
v360 (`4df0201`) continues the `cli.py` size-debt cleanup by moving read-only
discovery gold-set and RCA commands to `src/eidp/cli_discovery.py`. The
external top-level command names remain unchanged, including
`discovery-gold-set`, `eval-discovery-gold`, `summarize-discovery-evidence`,
and `discovery-rca-outcome-validate`. The DB-writing
`seed-discovery-gold-sites` command stays in `cli.py`, preserving the existing
write-lock AST gate. `cli.py` drops from `1405` lines to `997`; this is still
above the `<800` target, but the remaining excess is much smaller and isolated.

Verification: targeted discovery/RCA/write-lock/verifier tests passed with
`140 passed`; targeted Ruff and mypy passed; CLI help smoke for `eidp`,
`eidp eval-discovery-gold`, and `eidp discovery-rca-outcome-validate` showed
the expected commands. The full unit suite passed with `1367 passed, 5 warnings`.
`dist/eidp-windows-v360.zip` was rebuilt from clean commit `4df0201` with
SHA256 `637ab07fe953b676dd987e9224789343ffa00308aa19aece10f7eec91ac25555`.
The full non-Windows release gate returned `ok=true`; bounded evidence replay
returned `ok=true` with Tokyo `4` exact / `0` failures, Saitama `16` exact /
`0` failures, 聖十字 `1` exact / `0` failures, 更生 `1` exact / `0` failures,
中央情報 `1` exact / `0` failures, and 君津 `1` exact / `0` failures.
...
v279 (`1d5a612`) extends the same reproducibility rule to the URL-discovery
input layer. Web-search URL discovery now selects URL-missing active schools
in stable `prefecture, school_id` order, and both async and sync URL
verification select unverified `SchoolSite` rows by `school_id, site_id`.
This prevents the upstream URL seed set from drifting between bounded reruns
because of implicit database row order. The Windows distribution verifier now
requires the packaged URL-discovery stable-order contract. Verification:
`tests/unit` passed with `1270 passed, 5 warnings`,
`eval-discovery-gold --predictions data/discovery-gold-set/expected-predictions.jsonl
--fail-on-regression --json` reported `exact_matches=20`,
`failed_predictions=0`, and `unexpected_predictions=0`, and
`dist/eidp-windows-v279.zip` plus the latest alias `dist/eidp-windows.zip` both
passed `scripts/verify_windows_distribution.py --json` with SHA256
`645dc0dae9824014605bd818f1abc2e638d5eaa54fc9f7f90e257a22df89be2f`,
`git_commit=1d5a612e7eb4e56221f6e95fe0101b55f7f4f49d`, `git_dirty=false`,
`entry_count=3031`, `wheel_count=78`, `20` discovery gold-set entries,
`20` discovery gold expected predictions, and `47` downloadable supported
...
v281 (`3d2d6a`) makes discovery evidence loading tolerant of truncated JSONL
lines. `load_pdf_discovery_evidence` now skips malformed evidence rows with a
warning that includes the file path and line number, so a Ctrl-C, power loss,
or partial append at the tail of `discovery_evidence.jsonl` no longer prevents
the whole RCA summary from rendering. Valid rows before and after the malformed
line are still included. The Windows distribution verifier now requires the
packaged JSON decode guard. Verification: `tests/unit` passed with
`1273 passed, 5 warnings`,
`eval-discovery-gold --predictions data/discovery-gold-set/expected-predictions.jsonl
--fail-on-regression --json` reported `exact_matches=20`,
`failed_predictions=0`, and `unexpected_predictions=0`, and
`dist/eidp-windows-v281.zip` plus the latest alias `dist/eidp-windows.zip` both
passed `scripts/verify_windows_distribution.py --json` with SHA256
`b891faf827326e5c6d49b8ebe2a2d74d24664c5b87c047cbd291eee74299f527`,
`git_commit=3d2d6a97156c93f8aa98c274802f97e81925c787`, `git_dirty=false`,
`entry_count=3031`, `wheel_count=78`, `20` discovery gold-set entries,
`20` discovery gold expected predictions, and `47` downloadable supported
...
v283 (`c9ee449`) improves PDF candidate accuracy when a school page links the
same PDF more than once. Candidate extraction and cross-page candidate merging
now keep the stronger target-form anchor context for duplicate PDF URLs, so a
generic first link such as `PDF` no longer suppresses a later
`令和8年度 高等教育の修学支援新制度 確認申請書` link to the same file. Ties with the
same hint strength still keep the first occurrence, preserving stable ordering
and avoiding extra sibling-context noise. The Windows distribution verifier now
requires the packaged duplicate-anchor upgrade helper. Verification:
`tests/unit` passed with `1276 passed, 5 warnings`,
`eval-discovery-gold --predictions data/discovery-gold-set/expected-predictions.jsonl
--fail-on-regression --json` reported `exact_matches=20`,
`failed_predictions=0`, and `unexpected_predictions=0`, and
`dist/eidp-windows-v283.zip` plus the latest alias `dist/eidp-windows.zip` both
passed `scripts/verify_windows_distribution.py --json` with SHA256
`ca6a2bc19cd5330f48f9fe139ca30138315e56f6268856694cc8c67d96f0542c`,
`git_commit=c9ee44922b2f3ed9faa24f4e2c0f903fa5281738`, `git_dirty=false`,
`entry_count=3031`, `wheel_count=78`, `20` discovery gold-set entries,
`20` discovery gold expected predictions, and `47` downloadable supported
...
v293 (`c544a4e`) improves PDF candidate ordering for rolling target-year
discovery. Target-like candidates now carry an explicit download year rank, so
current target-FY form links are tried before stale or yearless form links even
when the stale link has a higher keyword score. The existing school-name rank
still remains ahead of the year rank, preserving group-page protection against
wrong-school PDFs. A regression test proves that a high-score FY2025 target
form no longer outranks an FY2026 target form, and the packaged verifier now
requires `_candidate_download_year_rank` to remain present in
`pdf_discovery.py`. Verification: `tests/unit` passed with `1287 passed,
...
`unexpected_predictions=0`. `dist/eidp-windows-v293.zip` plus the latest alias
`dist/eidp-windows.zip` both passed `scripts/verify_windows_distribution.py`
with SHA256 `098e0471feee045d513ccf1adb483d08a9c31f9d45f60f7e56abf39070c5f584`,
`git_commit=c544a4e78f0a74bda936c79dc6ef618d534dc847`, `git_dirty=false`,
`entry_count=3031`, `wheel_count=78`, `20` discovery gold-set entries,
`20` discovery gold expected predictions, and `47` downloadable supported
