...
Latest audited Windows package commit: `02ab507a347f9540e10d0d206c52f3d7b52751a0` (`eidp-windows-v73.zip`)
...
| Windows operator delivery | `dist/eidp-windows.zip` rebuilt as v73 at commit `02ab507`, verifier `ok=true`, `git_dirty=false`, SHA256 `fee2aa1b810acbdeb080fc0452174339b4559f3a6347f23cea04c6e79df5a448`, wheelhouse 82 wheels. Remote Windows smoke confirmed SHA256 match, `settings_page.py` present, no stale `"RC=-1"` launcher token, cached PDF rejection code, per-school fanout bounds, pre-download non-target filtering, decoded wrapper-URL hint filtering, and strong fiscal-year hint filtering present. Clean setup exit code 0 created DB with `school_count=2418` and `school_fiscal_year_status_count=2418`. | Package, setup smoke, official-index yield smoke, and a bounded Step 3 cache/prefilter smoke are covered. Full Windows PDF crawl/ingest and UI click-through are still not complete. |
...
- `uv run pytest -q` → `810 passed, 5 warnings`
