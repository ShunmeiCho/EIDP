...
Latest audited Windows package commit: `7a8d35a61eefb19d641b95ab6124016875d00b6d` (`eidp-windows-v92.zip`)
...
| Windows operator delivery | `dist/eidp-windows-v92.zip` rebuilt at commit `7a8d35a`, verifier `ok=true`, `git_dirty=false`, SHA256 `55e79777739ffb7d143655835a826bce04b50bb32455cccd5742fd9a31bdbf34`, wheelhouse 82 wheels. Latest package includes the current operator runbook, settings page, task-board settings shortcut, UI-first weekly rediscovery guidance, browser-proven non-transparent action buttons, bootstrap progress rejection breakdowns, URL-search provider/mode/limit visibility and package identity visibility on the task board, PDF確認 action-lane filtering, school-task source-chain CSV export, one-click `EIDP-diagnose.bat` diagnostics collection, 20-second auto-refresh while initial bootstrap is running, automatic recovery of stale Windows setup locks older than two hours, hardened ZIP/extracted-install gates for every navigated Streamlit route module, packaged runbook guidance that makes the package identity line the first screenshot check, and install-validator build identity fields for diagnostics-only feedback. Latest Mac smoke confirms Streamlit starts, settings page imports, Settings navigation has zero exceptions, task-board settings shortcut is covered by AppTest, and Chrome headless rendering shows `初回URL/PDF取得を開始` as a real styled button. Remote Windows smoke is still only proven through v73: SHA256 match, clean setup exit code 0, `school_count=2418`, `school_fiscal_year_status_count=2418`, official-index yield, and bounded Step 3 cache/prefilter behavior. | Latest package is locally verified and ready for Windows transfer. Full Windows PDF crawl/ingest and UI click-through are still not complete on v92. |
...
- `uv run pytest -q` → `835 passed, 5 warnings`
...
- `uv run python scripts/verify_windows_distribution.py dist/eidp-windows-v92.zip` → `OK core`, `git_commit=7a8d35a61eefb19d641b95ab6124016875d00b6d`, `git_dirty=false`, `sha256=55e79777739ffb7d143655835a826bce04b50bb32455cccd5742fd9a31bdbf34`, `entry_count=2994`, `wheel_count=82`, `prefecture_seed_rows=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_downloadable=47`
- `uv run python scripts/verify_windows_distribution.py dist/eidp-windows.zip` → `OK core`, `git_commit=7a8d35a61eefb19d641b95ab6124016875d00b6d`, `git_dirty=false`, `sha256=55e79777739ffb7d143655835a826bce04b50bb32455cccd5742fd9a31bdbf34`, `entry_count=2994`, `wheel_count=82`, `prefecture_seed_rows=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_downloadable=47`
- Extracted v92 ZIP smoke (`_temp/v92-extract-QwTutl`) using the packaged `scripts/validate_windows_install.py` → `OK install`, `build_commit=7a8d35a61eefb19d641b95ab6124016875d00b6d`, `build_branch=sprint8-handoff-finalize`, `build_dirty=false`, `master_xlsx_present=True`, `wheel_count=82`
...
2. Run latest v92 ZIP on Windows and verify:
