...
Latest audited Windows package commit: `76860d103fc25bedd2b75ee59c786028f4a213ef` (`eidp-windows-v101.zip`)
...
| Start from official government/prefecture indexes where possible | `data/prefecture-aggregators/seed.csv`; `src/eidp/scraper/prefecture_aggregator.py`; `scripts/verify_windows_distribution.py` now reads the ZIP seed/parser source and gates 47 prefecture rows, 47 parser registrations, and 47 downloadable official artifact URLs. Latest v101 verifier details: `prefecture_seed_rows=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_downloadable=47`, `prefecture_seed_with_school_link_signal=37`, `prefecture_seed_supplemental_rows=1`, `prefecture_seed_school_rows_total=2148`. Windows bounded bootstrap smoke on v73 downloaded/parsed all 47 official indexes and produced `official_index_rows_extracted=1948`, `official_index_rows_matched=1770`, `official_school_sites_added=1306`, plus 48 seed URLs and 295 corporation-pattern URLs before the 25-site PDF crawl. Latest Mac Saitama smoke parsed the official Saitama index into 58 extracted rows, 51 matched school rows, and 51 official school-site URLs before crawling 80 sites. | Release-gated for nationwide official-index bootstrap presence, and official-index URL yield is proven on Windows through Step 2b from v73. Latest v101 still needs Windows revalidation. Full target-year PDF crawling/ingestion still has to prove PDF yield. |
...
| Windows operator delivery | `dist/eidp-windows-v101.zip` rebuilt at commit `76860d1`, verifier `ok=true`, `git_dirty=false`, SHA256 `b9ef849c27243e2d54629d9c2b1025a70bf5218b0e9eefcfb538ddd95ba5d39e`, wheelhouse 82 wheels. `dist/eidp-windows.zip` has the same SHA256 and build info. The builder now writes `.sha256` sidecars automatically with portable relative paths. Latest package includes the current operator runbook, settings page, task-board settings shortcut, UI-first weekly rediscovery guidance, browser-proven non-transparent action buttons, bootstrap progress rejection breakdowns that preserve Step 3 discovery rejections after Step 4 ingest, URL-search provider/mode/limit visibility and package identity visibility on the task board, PDF確認 action-lane filtering and fiscal-year evidence summaries, crawler JSONL `year_evidence` / `detected_fiscal_year` fields surfaced in PDF確認, school-task source-chain CSV export, one-click `EIDP-diagnose.bat` diagnostics collection, 20-second auto-refresh while initial bootstrap is running, automatic recovery of stale Windows setup locks older than two hours, hardened ZIP/extracted-install gates for every navigated Streamlit route module, packaged runbook guidance that makes the package identity line the first screenshot check, install-validator build identity fields for diagnostics-only feedback, conservative URL/anchor target-year evidence for target confirmation forms whose PDF body omits the year, the v95 image-only false-positive guard for target-year admission guides, and AppTest coverage that locks the rolling 6-8 month publication-window guidance plus focused PDF確認 discovery-evidence rendering. Latest Mac smoke confirms Streamlit starts, settings page imports, Settings navigation has zero exceptions, task-board settings shortcut is covered by AppTest, Chrome headless rendering shows `初回URL/PDF取得を開始` as a real styled button, the task board explains the rolling 6-8 month publication window, PDF確認 focused evidence render has zero AppTest exceptions, and extracted v101 install validation passes. Remote Windows smoke is still only proven through v73: SHA256 match, clean setup exit code 0, `school_count=2418`, `school_fiscal_year_status_count=2418`, official-index yield, and bounded Step 3 cache/prefilter behavior. | Latest package is locally verified and ready for Windows transfer. Full Windows PDF crawl/ingest and UI click-through are still not complete on v101. |
...
- `uv run python scripts/build_windows_zip.py --skip-download --out-zip dist/eidp-windows-v101.zip` → wrote ZIP and automatic checksum sidecar
- `uv run python scripts/verify_windows_distribution.py dist/eidp-windows-v101.zip` → `OK core`, `git_commit=76860d103fc25bedd2b75ee59c786028f4a213ef`, `git_dirty=false`, `sha256=b9ef849c27243e2d54629d9c2b1025a70bf5218b0e9eefcfb538ddd95ba5d39e`, `entry_count=2994`, `wheel_count=82`, `prefecture_seed_rows=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_downloadable=47`
- `uv run python scripts/verify_windows_distribution.py dist/eidp-windows.zip` → `OK core`, `git_commit=76860d103fc25bedd2b75ee59c786028f4a213ef`, `git_dirty=false`, `sha256=b9ef849c27243e2d54629d9c2b1025a70bf5218b0e9eefcfb538ddd95ba5d39e`, `entry_count=2994`, `wheel_count=82`, `prefecture_seed_rows=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_downloadable=47`
- Extracted v101 ZIP smoke (`_temp/v101-extract-LBWMfe`) using the packaged `scripts/validate_windows_install.py` → `OK install`, `build_commit=76860d103fc25bedd2b75ee59c786028f4a213ef`, `build_branch=sprint8-handoff-finalize`, `build_dirty=false`, `master_xlsx_present=True`, `wheel_count=82`
...
2. Run latest v101 ZIP on Windows and verify:
