...
Latest audited Windows package commit: `9b02b64067d1401db0457243671eebe755134ba1` (`eidp-windows-v85.zip`)
...
| Windows operator delivery | `dist/eidp-windows-v85.zip` rebuilt at commit `9b02b64`, verifier `ok=true`, `git_dirty=false`, SHA256 `f6db838124af9235ca42e7aef2337eccbb3e3a17d8d4b0d6490a3d5b3ac36e09`, wheelhouse 82 wheels. Latest package includes the current operator runbook, settings page, UI-first weekly rediscovery guidance, browser-proven non-transparent action buttons, bootstrap progress rejection breakdowns, PDF確認 action-lane filtering, school-task source-chain CSV export, 20-second auto-refresh while initial bootstrap is running, and automatic recovery of stale Windows setup locks older than two hours. Latest Mac smoke confirms Streamlit starts, settings page imports, Settings navigation has zero exceptions, and Chrome headless rendering shows `初回URL/PDF取得を開始` as a real styled button. Remote Windows smoke is still only proven through v73: SHA256 match, clean setup exit code 0, `school_count=2418`, `school_fiscal_year_status_count=2418`, official-index yield, and bounded Step 3 cache/prefilter behavior. | Latest package is locally verified and ready for Windows transfer. Full Windows PDF crawl/ingest and UI click-through are still not complete on v85. |
...
- `uv run pytest -q` → `824 passed, 5 warnings`
...
- `uv run python scripts/verify_windows_distribution.py dist/eidp-windows-v85.zip` → `OK core`, `git_commit=9b02b64067d1401db0457243671eebe755134ba1`, `git_dirty=false`, `sha256=f6db838124af9235ca42e7aef2337eccbb3e3a17d8d4b0d6490a3d5b3ac36e09`, `entry_count=2992`, `wheel_count=82`, `prefecture_seed_rows=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_downloadable=47`
- `uv run python scripts/verify_windows_distribution.py dist/eidp-windows.zip` → `OK core`, `git_commit=9b02b64067d1401db0457243671eebe755134ba1`, `git_dirty=false`, `sha256=f6db838124af9235ca42e7aef2337eccbb3e3a17d8d4b0d6490a3d5b3ac36e09`, `entry_count=2992`, `wheel_count=82`, `prefecture_seed_rows=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_downloadable=47`
...
2. Run latest v85 ZIP on Windows and verify:
