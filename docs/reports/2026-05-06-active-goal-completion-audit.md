...
Latest audited Windows package commit: `83db29bc2608be04dd21ad554760e46f26c34f48` (`eidp-windows-v81.zip`)
...
| Windows operator delivery | `dist/eidp-windows-v81.zip` rebuilt at commit `83db29b`, verifier `ok=true`, `git_dirty=false`, SHA256 `db4d4e81cfd13a8eddc2a7e30a79b23682536ecdde008d80e0877ec1961af2e7`, wheelhouse 82 wheels. Latest package includes the current operator runbook, settings page, UI-first weekly rediscovery guidance, browser-proven non-transparent action buttons, and bootstrap progress rejection breakdowns. Latest Mac smoke confirms Streamlit starts, settings page imports, Settings navigation has zero exceptions, and Chrome headless rendering shows `初回URL/PDF取得を開始` as a real styled button. Remote Windows smoke is still only proven through v73: SHA256 match, clean setup exit code 0, `school_count=2418`, `school_fiscal_year_status_count=2418`, official-index yield, and bounded Step 3 cache/prefilter behavior. | Latest package is locally verified and ready for Windows transfer. Full Windows PDF crawl/ingest and UI click-through are still not complete on v81. |
...
- `uv run pytest -q` → `821 passed, 5 warnings`
...
- `uv run python scripts/verify_windows_distribution.py dist/eidp-windows-v81.zip` → `OK core`, `git_commit=83db29bc2608be04dd21ad554760e46f26c34f48`, `git_dirty=false`, `sha256=db4d4e81cfd13a8eddc2a7e30a79b23682536ecdde008d80e0877ec1961af2e7`, `entry_count=2992`, `wheel_count=82`, `prefecture_seed_rows=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_downloadable=47`
...
2. Run latest v81 ZIP on Windows and verify:
