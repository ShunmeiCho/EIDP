...
| Strict target-FY PDF discovery excludes stale fallback from success | Unit/gold-set coverage plus v463/v464 package lane; v459 bounded R7 weekly downloaded `2` target PDFs from `5` target-missing schools; v460 FY2026 Plan A selected no crawlable schools and second URL-rich FY2026 probe stopped without `last_run` | Mechanically guarded, but current FY2026/R8 production yield not proven |
