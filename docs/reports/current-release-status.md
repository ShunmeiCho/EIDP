...
Latest Mac/non-Windows package snapshot: `4ddc42b22cbf653eebefd6cc716c0bb0ed78aa20`
Current Mac-core-verifier-clean package for latest package snapshot: `dist/eidp-windows-v426.zip`
Latest Mac-core-verifier-clean package: `dist/eidp-windows-v426.zip`
Latest Mac-core package SHA256: `ac39b12304a0a611264334f5b106b998eb8a1b25018da2ae5c93c417256856dc`
Latest full non-Windows release-gate package: `dist/eidp-windows-v426.zip`
...
Current Stage 6 evidence draft: `docs/reports/eidp-v426-stage6-evidence-draft.md`

## Verdict

Status: **NOT COMPLETE**

v426 is the latest Mac/non-Windows release-gate-clean package. It was built
from package snapshot `4ddc42b22cbf653eebefd6cc716c0bb0ed78aa20`, which
includes the v424 configurable MEXT reconciliation school-type scope, the v425
PR1 operator hardening, and the v426 Mac-side hardening: Excel export thresholds
are read per call, proposal-review write helpers require an app lock, and Stage
6 residual cleanup refuses protected runtime files. The operator E2E
template remains package-neutral after the
rejected v421 build exposed hard-coded v420 package/SHA fields inside the
packaged template. Do not transfer v421. The v426 package was built with
`uv run python scripts/build_windows_zip.py --skip-download --out-zip
dist/eidp-windows-v426.zip --latest-alias`. The build wrote
`dist/eidp-windows-v426.zip`, `dist/eidp-windows-v426.zip.sha256`, and refreshed
`dist/eidp-windows.zip`. The release gate confirmed SHA256
`ac39b12304a0a611264334f5b106b998eb8a1b25018da2ae5c93c417256856dc` and
`dist/eidp-windows-v426.zip.sha256` carries the same value.
`scripts/verify_windows_distribution.py dist/eidp-windows-v426.zip` returned
`ok=true` inside the full release gate with
`git_commit=4ddc42b22cbf653eebefd6cc716c0bb0ed78aa20`, `git_dirty=false`,
...
sources. The v426 full gate also covers the launch-script localhost contract,
the URL-annotation URI filter, the operator review lock regressions, per-call
Excel threshold regression, and protected-runtime-file residual cleanup guard.
...
dist/eidp-windows-v426.zip --json --output logs/release-gate-v426-full.json`
returned `ok=true`. The recorded package/source freshness check reported
`package_commit=4ddc42b22cbf653eebefd6cc716c0bb0ed78aa20`,
`source_commit=4ddc42b22cbf653eebefd6cc716c0bb0ed78aa20`,
`source_dirty=false`, and `stale=false`; `unit_full` returned `1573 passed`; the
...
v426 has no Windows transfer/setup/UI proof yet because SSH-Win is currently
disconnected.

The docs-only stale-package replay path remains available for status-only
follow-up commits:
`uv run python scripts/run_non_windows_release_gates.py
dist/eidp-windows-v426.zip --skip-full-unit --allow-docs-only-stale-package
--json --output logs/release-gate-v426-docs-only-stale-after-status-refresh.json`.
Treat that as a
...
Mac-only v426 retroactive matrix gates passed for FY2025, FY2024, and FY2023
without SSH/Windows. The matrix run wrote
`logs/release-gate-v426-retroactive-matrix.json` with `ok=true` and
`case_count=3`. FY2025 wrote its fresh isolated export under
`_temp/non-windows-retroactive-fy2025-20260515-163006` and compared against
`_temp/v408-r7-cli-export.xlsx`; FY2024 wrote its fresh isolated export under
`_temp/non-windows-retroactive-fy2024-20260515-163126` and compared against
...
`_temp/non-windows-retroactive-fy2023-20260515-163305` and compared against
`_temp/non-windows-retroactive-fy2023-20260515-125526/output/retroactive-fy2023-export.xlsx`.
`logs/release-gate-v426-retroactive-fy2025-reference.json`,
`logs/release-gate-v426-retroactive-fy2024-reference.json`, and
`logs/release-gate-v426-retroactive-fy2023-reference.json` all returned
...
v426 is the next Windows execution candidate because it is the latest
...
real-cycle sign-off should use a fresh v426 transfer/setup lane unless the owner
explicitly freezes on v408. The v426 transfer checklist supports both
