...
Current Mac-verifier-clean package: `dist/eidp-windows-v389.zip`
Package commit: `5bd5fd811a8f5a64dc7caf178c1135872f492b6c`
Package SHA256: `ede9ffaf53d09167b8e8fe229ff0354696e2aa0374658c29cfc2449f0e832dd0`
Latest full non-Windows release-gate package: `dist/eidp-windows-v389.zip`
Latest Windows-core-validated package: `dist/eidp-windows-v389.zip`
...
The current Mac-verifier-clean ZIP snapshot is v389. It packages commit
`5bd5fd8`, which keeps the read-only `scripts/stage6_recovery_check.py` helper
...
packaged runbook. It also adds a `[stage6 recovery check]` section to
...
available and records OCR provenance in DB confidence breakdowns. The v389
versioned ZIP and latest alias both pass
`scripts/verify_windows_distribution.py --json`; the latest alias also passes
with the v383 OCR add-on and v106 Playwright add-on. The v389 core SHA256 is
`ede9ffaf53d09167b8e8fe229ff0354696e2aa0374658c29cfc2449f0e832dd0`, with
...
full non-Windows release gate is now v389 with `1407` unit tests, `44` exact
discovery gold-set predictions, and both package verifier modes passing
against SHA256
`ede9ffaf53d09167b8e8fe229ff0354696e2aa0374658c29cfc2449f0e832dd0`.
...
Latest v389 package-verifier commands:

- `uv run python scripts/build_windows_zip.py --skip-download --out-zip dist/eidp-windows-v389.zip --latest-alias`
  -> wrote `dist/eidp-windows-v389.zip` and refreshed `dist/eidp-windows.zip`;
  both have SHA256
  `ede9ffaf53d09167b8e8fe229ff0354696e2aa0374658c29cfc2449f0e832dd0`.
- `uv run python scripts/verify_windows_distribution.py --json dist/eidp-windows-v389.zip`
  -> `ok=true`, commit `5bd5fd811a8f5a64dc7caf178c1135872f492b6c`,
...
- `uv run python scripts/verify_windows_distribution.py --json dist/eidp-windows.zip --ocr-addon dist/eidp-ocr-addon-windows-v383-smoke.zip --playwright-addon dist/eidp-playwright-addon-windows-v106.zip`
  -> `OK core`, `OK ocr-addon`, and `OK playwright-addon`, with matching v389
...
Latest v389 full non-Windows release-gate commands:

- `uv run python scripts/run_non_windows_release_gates.py dist/eidp-windows-v389.zip --json --output _temp/v389-non-windows-release-gates-full.json`
  -> `ok=true`; SHA256 sidecar matched
  `ede9ffaf53d09167b8e8fe229ff0354696e2aa0374658c29cfc2449f0e832dd0`; full
  unit passed with `1407 passed, 5 warnings`; validator/distribution unit tests
  passed with `144 passed`; validator/distribution mypy and Ruff passed;
  ...
  package verifier modes passed with the same v389 SHA256.
