...
Current Mac-verifier-clean package: `dist/eidp-windows-v391.zip`
Package commit: `ee39ee8cb860fe6e61da4ba66448fbd77f8d9e8a`
Package SHA256: `2dded325c2983daa457a006bcc1bc892af7a054e9802a17d3d476b9c9fe5b7db`
Latest full non-Windows release-gate package: `dist/eidp-windows-v391.zip`
Latest Windows-core-validated package: `dist/eidp-windows-v391.zip`
...
The current Mac-verifier-clean ZIP snapshot is v391. It packages commit
`ee39ee8`, which adds a root-level `EIDP-stage6-recovery.bat` launcher so a
non-technical operator can collect the read-only Stage 6 recovery JSON without
browsing into `scripts\`. It preserves the v390 Stage 6 operator E2E template
...
action-match status, residual interrupted-smoke paths, recommendations, and
optional `logs\stage6-recovery-*.json` attachment. It keeps the read-only
...
available and records OCR provenance in DB confidence breakdowns. The v391
versioned ZIP and latest alias both pass
`scripts/verify_windows_distribution.py --json`; the latest alias also passes
with the v383 OCR add-on and v106 Playwright add-on. The v391 core SHA256 is
`2dded325c2983daa457a006bcc1bc892af7a054e9802a17d3d476b9c9fe5b7db`, with
`44` discovery gold-set entries, `17` publication-lag cases, `47` prefecture
seeds, `entry_count=3066`, and `undemonstrated_pattern_sources=[]`. The latest
full non-Windows release gate is now v391 with `1409` unit tests, `44` exact
discovery gold-set predictions, and both package verifier modes passing
against SHA256
`2dded325c2983daa457a006bcc1bc892af7a054e9802a17d3d476b9c9fe5b7db`.
...
Latest v391 package-verifier commands:

- `uv run python scripts/build_windows_zip.py --skip-download --out-zip dist/eidp-windows-v391.zip --latest-alias`
  -> wrote `dist/eidp-windows-v391.zip` and refreshed `dist/eidp-windows.zip`;
  both have SHA256
  `2dded325c2983daa457a006bcc1bc892af7a054e9802a17d3d476b9c9fe5b7db`.
- `uv run python scripts/verify_windows_distribution.py --json dist/eidp-windows-v391.zip`
  -> `ok=true`, commit `ee39ee8cb860fe6e61da4ba66448fbd77f8d9e8a`,
  `build_dirty=false`, `entry_count=3066`, `wheel_count=78`,
  root-level `EIDP-stage6-recovery.bat`, and
  ...
  requires `stage6_recovery_rc` and scheduled-task recovery evidence fields.
...
- `uv run python scripts/verify_windows_distribution.py --json dist/eidp-windows.zip --ocr-addon dist/eidp-ocr-addon-windows-v383-smoke.zip --playwright-addon dist/eidp-playwright-addon-windows-v106.zip`
  -> `OK core`, `OK ocr-addon`, and `OK playwright-addon`, with matching v391
  ...
  `db-backup --output $dbBackup`, `VACUUM INTO`,
  `PRAGMA wal_checkpoint(TRUNCATE)`, and the Stage 6 recovery root/helper
  commands.
...
Latest v391 full non-Windows release-gate commands:

- `uv run python scripts/run_non_windows_release_gates.py dist/eidp-windows-v391.zip --json --output _temp/v391-non-windows-release-gates-full.json`
  -> `ok=true`; SHA256 sidecar matched
  `2dded325c2983daa457a006bcc1bc892af7a054e9802a17d3d476b9c9fe5b7db`; full
  unit passed with `1409 passed, 5 warnings`; validator/distribution unit tests
  passed with `146 passed`; validator/distribution mypy and Ruff passed;
  ...
  package verifier modes passed with the same v391 SHA256.
