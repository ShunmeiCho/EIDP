...
Current Mac-verifier-clean package: `dist/eidp-windows-v392.zip`
Package commit: `597337b7534259072b7f3cd599067e59bef58012`
Package SHA256: `3cdf8921bd9004bf68e83cf17754cb219ea53a637a9cc9769459ae98f20cfa1a`
Latest full non-Windows release-gate package: `dist/eidp-windows-v392.zip`
Latest Windows-core-validated package: `dist/eidp-windows-v392.zip`
...
The current Mac-verifier-clean ZIP snapshot is v392. It packages commit
`597337b`, which adds a root-level `EIDP-stage6-evidence.bat` launcher, the
...
WAL/SHM files, PDFs, wheelhouse, and runtime directories. v392 preserves the
...
available and records OCR provenance in DB confidence breakdowns. The v392
versioned ZIP and latest alias both pass
`scripts/verify_windows_distribution.py --json`; the latest alias also passes
with the v383 OCR add-on and v106 Playwright add-on. The v392 core SHA256 is
`3cdf8921bd9004bf68e83cf17754cb219ea53a637a9cc9769459ae98f20cfa1a`, with
`44` discovery gold-set entries, `17` publication-lag cases, `47` prefecture
seeds, `entry_count=3069`, and `undemonstrated_pattern_sources=[]`. The latest
full non-Windows release gate is now v392 with `1415` unit tests, `44` exact
discovery gold-set predictions, and both package verifier modes passing
against SHA256
`3cdf8921bd9004bf68e83cf17754cb219ea53a637a9cc9769459ae98f20cfa1a`.
...
Latest v392 package-verifier commands:

- `uv run python scripts/build_windows_zip.py --skip-download --out-zip dist/eidp-windows-v392.zip --latest-alias`
  -> wrote `dist/eidp-windows-v392.zip` and refreshed `dist/eidp-windows.zip`;
  both have SHA256
  `3cdf8921bd9004bf68e83cf17754cb219ea53a637a9cc9769459ae98f20cfa1a`.
- `uv run python scripts/verify_windows_distribution.py --json dist/eidp-windows-v392.zip`
  -> `ok=true`, commit `597337b7534259072b7f3cd599067e59bef58012`,
  `build_dirty=false`, `entry_count=3069`, `wheel_count=78`,
...
- `uv run python scripts/verify_windows_distribution.py --json dist/eidp-windows.zip --ocr-addon dist/eidp-ocr-addon-windows-v383-smoke.zip --playwright-addon dist/eidp-playwright-addon-windows-v106.zip`
  -> `OK core`, `OK ocr-addon`, and `OK playwright-addon`, with matching v392
...
Latest v392 full non-Windows release-gate commands:

- `uv run python scripts/run_non_windows_release_gates.py dist/eidp-windows-v392.zip --json --output _temp/v392-non-windows-release-gates-full.json`
  -> `ok=true`; SHA256 sidecar matched
  `3cdf8921bd9004bf68e83cf17754cb219ea53a637a9cc9769459ae98f20cfa1a`; full
  unit passed with `1415 passed, 5 warnings`; validator/distribution unit tests
  ...
  package verifier modes passed with the same v392 SHA256.
