...
Current Mac-verifier-clean package for latest code evidence base: `dist/eidp-windows-v407.zip`
Current package SHA256: `af48ed37d65695c044b520da78aad5307ed89b4b4a38cf27c6dc7e2737f50940`
...
v407 is now the current clean package lane for source commit
...
A non-Excel proof of that browser download was copied into the Windows v407
logs as
`logs\diagnostics-v407-r7-browser-excel-proof-20260515-024000.txt`. A refreshed
non-Excel evidence bundle, `logs\stage6-evidence-20260514-174426.zip`, was then
created with `included_count=8` and verified with `--require-label last_run`:
`ok=true`, `entry_count=9`, `present_labels=["build_info", "diagnostics",
"last_run", "stage6_recovery", "stage6_residual_cleanup", "weekly_run_logs"]`,
`forbidden_entries=[]`, `unsafe_entries=[]`, and `errors=[]`. The manifest still
records missing `bootstrap_logs`, `bootstrap_progress`, and `discovery_rca`, so
this remains diagnostic evidence, but it now carries the R7 browser Excel proof
without bundling the Excel file itself.
...
`logs\stage6-evidence-20260514-174426.zip` carrying the R7 browser Excel proof,
plus the disposable sandbox browser proof for `PDF確認・手入力`, fiscal-year
override, Excel preview generation, `監査ログ`, and outbox flush.
...
diagnostic until the scheduled-task action is confirmed, do not externally share
any bundle containing Excel exports, and do not run
...
| 3. 証跡採取コマンド | v407 hash/setup/validate/release-gate/export/UI-health and R7 browser Excel proof is available; dry-run `last_run.json` was included in verifier-accepted diagnostic bundle `logs\stage6-evidence-20260514-174426.zip` | `EIDP-diagnose.bat` after the click-through cycle; final evidence bundle from the real operator cycle |
...
v407 supersedes the older ZIP distribution/setup/UI-health rows; full
...
Current v407 package-verifier commands:
