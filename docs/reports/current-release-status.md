...
Current Mac-verifier-clean package: `dist/eidp-windows-v314.zip`
Package commit: `08639e37767f785c86b6e53323f0b4cb859c169f`
Package SHA256: `c7294a3540df126079663964bfcdab0db8cdbe686a28eccb84be4bd7b7437983`
...
| --- | --- | --- |
| 47 prefecture official indexes seed school public URLs | v314 verifier: `prefecture_seed_rows=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_downloadable=47`, `prefecture_seed_school_rows_total=2148` | Evidence present |
| Discover and download current target-FY PDFs in strict mode | v314 verifier clean; discovery gold-set `20` entries; expected predictions `20/20 exact`; package exposes `discovery_gold_undemonstrated_pattern_sources` so speculative extractor coverage is visible | Partially proven |
...
| Extract with pdfplumber/PyMuPDF/Tesseract and write only confidence >= 0.70 rows | Unit/package gates cover OCR runtime presence and confidence contracts, but no current Windows operator E2E has revalidated the whole extraction-to-Excel path on v314 | Partially proven |
...
| Excel template output | Package verifier includes Excel/export contracts; current operator-PC preview/download flow is not revalidated on v314 | Partially proven |
| ManualActionLog audit for operator actions | Package verifier includes audit contracts and outbox checks; current operator-PC run not revalidated on v314 | Partially proven |
| ZIP distribution, double-click setup, browser UI offline operation | v314 ZIP verifies clean on macOS packaging gate; latest Windows setup-verified evidence is older than v314 | Missing current Win proof |
...
Commands run for v314:

- `uv run pytest tests/unit -q` -> `1312 passed, 5 warnings`
...
- `uv run python scripts/verify_windows_distribution.py dist/eidp-windows-v314.zip` -> `OK core`
- `uv run eidp discovery-gold-set --json` -> `pattern_source_counts={"wordpress_download_manager": 1}`

v314 verifier exposes the current demonstration gap:
