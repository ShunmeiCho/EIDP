...
Current Mac-verifier-clean package: `dist/eidp-windows-v387.zip`
Package commit: `45a6d4c0cdf5d99b9da65a691e7bf80f00daacf1`
Package SHA256: `d0030c5260d4078ddb58ec7aea440da00b07e1b7e74977b10b74f9df66174196`
Latest full non-Windows release-gate package: `dist/eidp-windows-v387.zip`
Latest Windows-core-validated package: `dist/eidp-windows-v387.zip`
...
The current Mac-verifier-clean ZIP snapshot is v387. It packages commit
`45a6d4c`, which keeps the read-only `scripts/stage6_recovery_check.py` helper
in the Windows ZIP, adds a Windows-local `scripts\stage6_recovery_check.bat`
wrapper for SSH-unavailable recovery, and records that path in the packaged
runbook. It preserves the v384 image-PDF ingest path that routes through the
packaged/system Tesseract TSV wrapper when available and records OCR provenance
in DB confidence breakdowns. The v387 versioned ZIP and latest alias both pass
`scripts/verify_windows_distribution.py --json`; the latest alias also passes
with the v383 OCR add-on and v106 Playwright add-on. The v387 core SHA256 is
`d0030c5260d4078ddb58ec7aea440da00b07e1b7e74977b10b74f9df66174196`, with
...
full non-Windows release gate is now v387 with `1406` unit tests, `44` exact
discovery gold-set predictions, and both package verifier modes passing
against SHA256
`d0030c5260d4078ddb58ec7aea440da00b07e1b7e74977b10b74f9df66174196`.
...
Latest v387 package-verifier commands:

- `uv run python scripts/build_windows_zip.py --skip-download --out-zip dist/eidp-windows-v387.zip --latest-alias`
  -> wrote `dist/eidp-windows-v387.zip` and refreshed `dist/eidp-windows.zip`;
  both have SHA256
  `d0030c5260d4078ddb58ec7aea440da00b07e1b7e74977b10b74f9df66174196`.
- `uv run python scripts/verify_windows_distribution.py --json dist/eidp-windows-v387.zip`
  -> `ok=true`, commit `45a6d4c0cdf5d99b9da65a691e7bf80f00daacf1`,
...
- `uv run python scripts/verify_windows_distribution.py --json dist/eidp-windows.zip --ocr-addon dist/eidp-ocr-addon-windows-v383-smoke.zip --playwright-addon dist/eidp-playwright-addon-windows-v106.zip`
  -> `OK core`, `OK ocr-addon`, and `OK playwright-addon`, with matching v387
...
Latest v387 full non-Windows release-gate commands:

- `uv run python scripts/run_non_windows_release_gates.py dist/eidp-windows-v387.zip --json --output _temp/v387-non-windows-release-gates-full.json`
  -> `ok=true`; SHA256 sidecar matched
  `d0030c5260d4078ddb58ec7aea440da00b07e1b7e74977b10b74f9df66174196`; full
  ...
  package verifier modes passed with the same v387 SHA256.
