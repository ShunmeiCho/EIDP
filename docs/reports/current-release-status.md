...
Current Mac-verifier-clean package: `dist/eidp-windows-v319.zip`
Package commit: `9dab69bb5daf1034ef8feaedbe1ca9677f9c6184`
Package SHA256: `6c5d671b6927b652d2c7baa5ce3dba97a0b4908d372c42934525322456ab48ce`
...
| --- | --- | --- |
| 47 prefecture official indexes seed school public URLs | v319 verifier: `prefecture_seed_rows=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_downloadable=47`, `prefecture_seed_school_rows_total=2148` | Evidence present |
| Discover and download current target-FY PDFs in strict mode | v319 verifier clean; discovery gold-set `20` entries; expected predictions `20/20 exact`; package exposes `discovery_gold_undemonstrated_pattern_sources` and the CLI can fail on undemonstrated extractor sources | Partially proven |
...
| Extract with pdfplumber/PyMuPDF/Tesseract and write only confidence >= 0.70 rows | Unit/package gates cover OCR runtime presence and confidence contracts, but no current Windows operator E2E has revalidated the whole extraction-to-Excel path on v319 | Partially proven |
...
| Excel template output | Package verifier includes Excel/export contracts; current operator-PC preview/download flow is not revalidated on v319 | Partially proven |
| ManualActionLog audit for operator actions | Package verifier includes audit contracts and outbox checks; current operator-PC run not revalidated on v319 | Partially proven |
| ZIP distribution, double-click setup, browser UI offline operation | v319 ZIP verifies clean on macOS packaging gate; latest Windows setup-verified evidence is older than v319 | Missing current Win proof |
| Shipping threshold: true target PDF 60-70% auto-acquired, operator manual work <=30% | v319 includes `eidp report ship-readiness --fail-on-missing-goal` and records it in `EIDP-diagnose.bat`; current local DB is not initialized, so real nationwide result remains unmeasured | Missing |

## Current Non-Windows Evidence

Commands run for v319:
...
- `uv run python scripts/verify_windows_distribution.py dist/eidp-windows-v319.zip` -> `OK core`
...
v319 verifier exposes the current demonstration gap:
