...
Current Mac-verifier-clean package: `dist/eidp-windows-v390.zip`
Package commit: `60d2348e036730c0bc3b521a8ce7f16144f57223`
Package SHA256: `932a24ef3fbee8e95ad70319e0a9d4a030adef45085bd8c65c56adc590a7eccd`
Latest full non-Windows release-gate package: `dist/eidp-windows-v390.zip`
Latest Windows-core-validated package: `dist/eidp-windows-v390.zip`
...
The current Mac-verifier-clean ZIP snapshot is v390. It packages commit
`60d2348`, which requires the Stage 6 operator E2E template to preserve the
`[stage6 recovery check]` diagnostics evidence: `stage6_recovery_rc`,
scheduled-task execute path, expected action, action-match status, residual
interrupted-smoke paths, recommendations, and optional
`logs\stage6-recovery-*.json` attachment. It keeps the read-only
...
available and records OCR provenance in DB confidence breakdowns. The v390
versioned ZIP and latest alias both pass
`scripts/verify_windows_distribution.py --json`; the latest alias also passes
with the v383 OCR add-on and v106 Playwright add-on. The v390 core SHA256 is
`932a24ef3fbee8e95ad70319e0a9d4a030adef45085bd8c65c56adc590a7eccd`, with
`44` discovery gold-set entries, `17` publication-lag cases, `47` prefecture
seeds, `entry_count=3065`, and `undemonstrated_pattern_sources=[]`. The latest
full non-Windows release gate is now v390 with `1408` unit tests, `44` exact
discovery gold-set predictions, and both package verifier modes passing
against SHA256
`932a24ef3fbee8e95ad70319e0a9d4a030adef45085bd8c65c56adc590a7eccd`.
...
Latest v390 package-verifier commands:

- `uv run python scripts/build_windows_zip.py --skip-download --out-zip dist/eidp-windows-v390.zip --latest-alias`
  -> wrote `dist/eidp-windows-v390.zip` and refreshed `dist/eidp-windows.zip`;
  both have SHA256
  `932a24ef3fbee8e95ad70319e0a9d4a030adef45085bd8c65c56adc590a7eccd`.
- `uv run python scripts/verify_windows_distribution.py --json dist/eidp-windows-v390.zip`
  -> `ok=true`, commit `60d2348e036730c0bc3b521a8ce7f16144f57223`,
  `build_dirty=false`, `entry_count=3065`, `wheel_count=78`, and
...
- `uv run python scripts/verify_windows_distribution.py --json dist/eidp-windows.zip --ocr-addon dist/eidp-ocr-addon-windows-v383-smoke.zip --playwright-addon dist/eidp-playwright-addon-windows-v106.zip`
  -> `OK core`, `OK ocr-addon`, and `OK playwright-addon`, with matching v390
  ...
  `PRAGMA wal_checkpoint(TRUNCATE)`, and the Stage 6 recovery helper command.
...
Latest v390 full non-Windows release-gate commands:

- `uv run python scripts/run_non_windows_release_gates.py dist/eidp-windows-v390.zip --json --output _temp/v390-non-windows-release-gates-full.json`
  -> `ok=true`; SHA256 sidecar matched
  `932a24ef3fbee8e95ad70319e0a9d4a030adef45085bd8c65c56adc590a7eccd`; full
  unit passed with `1408 passed, 5 warnings`; validator/distribution unit tests
  passed with `145 passed`; validator/distribution mypy and Ruff passed;
  ...
  package verifier modes passed with the same v390 SHA256.
