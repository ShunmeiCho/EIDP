...
Current Mac-verifier-clean package: `dist/eidp-windows-v334.zip`
Package commit: `bb42896d9134280a19ee4f1abb0e4cadbda5e542`
Package SHA256: `7d4043b4d78f669c291795482eb32791a862a02cf8ba3763444aa674e2f88ea2`
...
The current source/ZIP snapshot is v334 and passes the default macOS package
verifier. v334 keeps the v326 strict-mode fix for opaque WordPress Download
...
prefecture-index-only auto successes as operator-review candidates. Windows
setup, SQLite initialization, diagnostics, and a bounded 50-site Saitama
bootstrap are still proven on v333, not yet rerun on v334. The product goal is
...
| --- | --- | --- |
| 47 prefecture official indexes seed school public URLs | v334 verifier: `prefecture_seed_rows=47`, `prefecture_seed_parser_supported=47`, `prefecture_seed_downloadable=47`, `prefecture_seed_school_rows_total=2148`; Windows v333 Saitama run downloaded the current official artifact and added `51` `SchoolSite` rows from `58` extracted / `51` matched rows | Evidence present |
| Discover and download current target-FY PDFs in strict mode | v334 verifier clean by default; discovery gold-set `23` entries; Windows v333 Saitama 50-site run crawled `50` official-index sites, found candidates on `49`, downloaded `0`, processed `0`, and produced `0` Excel-ready schools after removing false-positive prefecture-index year fill | Mechanically proven, strict yield failing |
...
| Excel template output | v334 package verifier includes Excel/export contracts and centralized confidence threshold contract; current operator-PC preview/download flow is not revalidated on v333/v334 | Partially proven |
| ManualActionLog audit for operator actions | v334 package verifier includes audit contracts and outbox checks; current operator-PC run not revalidated through browser UI on v333/v334 | Partially proven |
| ZIP distribution, double-click setup, browser UI offline operation | v334 ZIP verifies clean on macOS packaging gate; v333 was transferred to Windows with matching SHA256, extracted to `C:\Users\cyo20\EIDP-v333-422741d`, and `scripts\first_setup.bat` completed successfully; browser UI click-through remains unverified | Backend Win setup proof present, UI proof missing |
...
Commands run for v334 source/package:
...
- `uv run python scripts/verify_windows_distribution.py dist/eidp-windows-v334.zip` -> `OK core`
- `uv run python scripts/verify_windows_distribution.py dist/eidp-windows-v334.zip --require-demonstrated-discovery-patterns` -> expected failure for the six remaining undemonstrated sources
...
v334 verifier exposes the current demonstration gap:
